...
export function PlayerPill({ p, tone = 'slate' }: { p: any; tone?: 'give' | 'get' | 'slate' }) {
  ...
  return (
    <button
      onClick={() => open(p.id)}
      title={`Week ${p.matchup?.week ?? '?'}: ${p.current_week_ppg ?? p.adj_ppg ?? '?'} pts · ROS ${p.ros_ppg ?? p.ppg ?? '?'} ppg · active ${p.active_probability == null ? '?' : Math.round(p.active_probability * 100) + '%'} · ${valueLabel(p, 'market')} ${p.value?.toLocaleString() ?? '?'}${p.bye ? ` · bye ${p.bye}` : ''}`}
