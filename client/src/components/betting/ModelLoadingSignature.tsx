...
export function ModelLoadingSignature({ sport, stages = DEFAULT_STAGES, compact = false }: {
  sport: 'NFL' | 'MLB'; stages?: string[]; compact?: boolean;
}) {
  ...
  return <div className={`card overflow-hidden ${compact ? 'p-4' : 'p-6'}`} role="status" aria-live="polite">
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0 flex-1"><div className="text-sm font-black text-slate-900">Preparing {sport} model intelligence</div><div className="mt-0.5 text-xs text-slate-500">Evidence-first load · {elapsed.toFixed(1)}s</div></div>
