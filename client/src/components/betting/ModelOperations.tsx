...
export function NflModelOperations() {
  ...
  const passingGates = d.gates.filter(gate => gate.passed).length;
  const firstFailedGate = d.gates.find(gate => !gate.passed);
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap items-center gap-2"><StatusPill tone={d.verdict === 'promotion_eligible' ? 'good' : 'warn'}>{passingGates}/{d.gates.length} profit gates pass</StatusPill>
      {firstFailedGate && <span className="text-xs text-slate-500">First unmet gate: {firstFailedGate.label} ({show(firstFailedGate.actual)}; target {firstFailedGate.target}).</span>}
...
function GateGrid({ gates }: { gates: Gate[] }) { return <section><SectionHeading eyebrow="Promotion gates" title={`${gates.filter(x => x.passed).length}/${gates.length} currently pass`} description="A challenger remains research-only until every target is met at the same immutable cutoff." /><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{gates.map(g => <div key={g.id} className="card p-4"><div className="flex items-center gap-2"><StatusPill tone={g.passed ? 'good' : 'warn'}>{g.passed ? 'pass' : 'target not met'}</StatusPill><div className="font-semibold text-slate-900">{g.label}</div></div><div className="mt-3 text-sm font-black text-slate-800">{show(g.actual)}</div><div className="mt-1 text-xs text-slate-500">Target: {g.target}</div></div>)}</div></section>; }
