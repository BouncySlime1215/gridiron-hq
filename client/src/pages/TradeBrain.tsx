...
import { useWarRoom, useWarRoomClones } from '../components/warroom/useWarRoom';
...
export default function TradeBrain() {
  ...
  // UI-ENG-4: the clone panel's read, asked for only once the War Room says it is on.
  const clones = useWarRoomClones(warOn ? activeId : null);
  ...
  if (tab === 'war-room' && warOn && activeId && warRoom.data) {
    return (
      <WarRoom view={warRoom.data} clones={clones.data} activeId={activeId} onLeague={setActiveId} onExit={setTab}
