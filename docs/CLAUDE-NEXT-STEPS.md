...
- **Correction and continuation, later the same night (2026-09-14/15): the recovered corpus was NOT permanently gone.** The `~/Documents` read permission that blocked this session was fixed and the session restarted; the full Codex handoff is readable at `/Users/nick_matta/Documents/Codex/2026-09-14/saved-2-memories-ran-a-command/outputs/` — `GRIDIRON-MASTER-PLAN.md` (the complete document this file's Codex bullets above were reconciling from a pasted copy of, now confirmed byte-for-byte the same plan), `README-CLAUDE-HANDOFF.md`, `AGENT-PLAYBOOK.md`, 20 WP prompts under `agent-prompts/`, and `recovered-evidence/` containing the full 59-document `audit/`, 41-document `audit-system/`, 70-document `research2/` collections, `plans/` (Giant Plan, What Next), and a `sept14/` folder of fresh read-only live-DB verification notes that supersede the three historical-snapshot leads above:
  ...
  - **Two more slices in flight as background agents, not yet landed:** the `nfl_qbr_weekly` bulk-seed root-cause investigation and fix (immediate next action, below), and migrating the 7 remaining inline `fold_cutoff` duplications in `market_lab.py`/`tree_lab.py` onto the shared `dataset.py::eligible_training_rows` (governance manual §4.4 item 2). Do not mark either `tested` here until landed and independently reviewed the same way as the five above.

**Immediate next actions**, in order: (1) review and land the five in-flight agent fixes above once they report back, updating §0.1's status register with real evidence for each; (2) fix the confirmed-live `nfl_qbr_weekly` bulk-seed/duplicate-identity problem — both the upsert-key fix (`season,week,player_id`, not team) and finding why weeks 2–18 were ever populated with copied 2025 data; (3) investigate the `pass_yds` regression / `interception` negative skill `runModelWatch()` flagged; (4) Stage 3 — fit an actual team-strength model on `build_football_dataset`'s 7,291-row history and compare it against the existing rating systems in `nfl-ensemble.js`, per the master plan's WP06/WP11-12; (5) migrate the seven remaining inline `fold_cutoff` computations in `market_lab.py`/`tree_lab.py`'s `run()` functions onto `dataset.py::fold_cutoff`/`eligible_training_rows` (governance manual §4.4 item 2, still open); (6) work through the master plan's WP03-WP05 (canonical identities, immutable record contracts, source coverage admission) now that the full 20-WP spec is readable — these are the plan's own stated dependencies before any new model training; (7) decide the disposition of the other ~28 stale-season defaults one at a time; (8) connect `trial-statistics.js`'s overfitting checks to a real gate (candidate: `audit-registry.js`'s promotion path) instead of leaving them report-only.
