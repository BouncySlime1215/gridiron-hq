...
- **Correction and continuation, later the same night (2026-09-14/15): the recovered corpus was NOT permanently gone.** The `~/Documents` read permission that blocked this session was fixed and the session restarted; the full Codex handoff is readable at `/Users/nick_matta/Documents/Codex/2026-09-14/saved-2-memories-ran-a-command/outputs/` — `GRIDIRON-MASTER-PLAN.md` (the complete document this file's Codex bullets above were reconciling from a pasted copy of, now confirmed byte-for-byte the same plan), `README-CLAUDE-HANDOFF.md`, `AGENT-PLAYBOOK.md`, 20 WP prompts under `agent-prompts/`, and `recovered-evidence/` containing the full 59-document `audit/`, 41-document `audit-system/`, 70-document `research2/` collections, `plans/` (Giant Plan, What Next), and a `sept14/` folder of fresh read-only live-DB verification notes that supersede the three historical-snapshot leads above:
  ...
  - **Nine more fixes landed and pushed the same night, `c63bc19` through `0c29c1b`**, each personally reviewed (diff read, targeted + full suite rerun) before commit:
    ...
    - **Dead-code cleanup:** removed a stale `'elo'` entry from `nfl-expert-council.js`'s `simpleIds` set — `nfl-ensemble.js` registers the Elo-family rating as `'melo'` (already present), so `'elo'` never matched anything; a pure no-op on the computed value, just removing a misleading dead reference. A related but NOT fixed finding — `playerOpportunity` computed at the same file's line ~509 and never wired into its own expert's `output(...)` call — was deliberately left alone and flagged separately for human review (spawned task `task_4283df35`) rather than silently changed, since wiring it in would give a currently-always-null expert a real forecast, which is a behavior change needing evidence/product sign-off, not a bug fix.
  ...
  - **Total for the night: 14 confirmed, independently-verified fixes landed and pushed** (`994d32e` through `0c29c1b`), plus 2 honest no-defect-found investigations and 1 flagged-for-human-decision item. Every fix was reviewed personally against its actual diff and test output before committing — none taken on a dispatching agent's report alone. Full suite after the final commit: **2001 pass, 0 fail, 39 skip.**

**Immediate next actions**, in order: (1) investigate the `pass_yds` regression / `interception` negative skill `runModelWatch()` flagged (a genuine model-weakness finding, not a code bug — belongs to Stage 6's measured-weakness process, not a quick fix); (2) resolve the flagged `player_opportunity` forecast-wiring question (`task_4283df35`); (3) Stage 3 — fit an actual team-strength model on `build_football_dataset`'s 7,291-row history and compare it against the existing rating systems in `nfl-ensemble.js`, per the master plan's WP06/WP11-12; (4) work through the master plan's WP03-WP05 (canonical identities, immutable record contracts, source coverage admission) now that the full 20-WP spec is readable — these are the plan's own stated dependencies before any new model training; (5) decide the disposition of the other ~28 stale-season defaults one at a time; (6) the C08 finding (unresolved-team/unverified evidence admitted into `nfl-t60-packet.js`'s injury/news coverage counts) was reviewed tonight and found to already carry extensive deliberate design reasoning in its own comments (explicitly disclosed, not an oversight) — deprioritized rather than force-fixed; revisit only as part of the fuller WP03/WP09 identity-resolution work, not in isolation.
