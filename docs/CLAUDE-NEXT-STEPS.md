...
- **Overfitting/safeguard correction, verified against code today:** `server/services/trial-statistics.js` already implements the correct fix for the specific flaw the recovered research flagged (a naive Šidák/Holm correction overstating independent evidence across *correlated* retries) — Geyer (1992) effective-trial-count, Bailey & López de Prado's Deflated Sharpe Ratio, and CSCV probability-of-backtest-overfitting, all tested (`test/trial-statistics.test.js`, 15/15 passing). It is **not wired into anything that gates a real decision** — it only feeds `scripts/run-purged-evaluation.mjs`, a report generator. Closing that gap (a real consumer that can actually block a promotion) is unclaimed work, not done work.
- **"Missing 2026 team features," verified and fixed today:** ~30 services across the codebase default their `seasons` parameter to a hardcoded array ending at 2025 (last season), silently excluding the live 2026 season for any caller that doesn't pass an explicit override. Two of these are live production monitoring loops, not just research helpers, and both were fixed with regression tests today: `nfl-data-consistency.js` (the cross-season coverage/corruption audit) and `nfl-model-watch.js` (the daily drift-detection loop wired into `scheduler.js`'s `refreshModelWatch`) — both were structurally blind to the entire current season until this fix. The other ~28 occurrences (mostly backtest/research defaults, e.g. `consensus-weights.js`, `line-move-study.js`, `nfl-*-audit.js`) were deliberately **not** touched in this pass: several of them are fixed historical-cohort windows tied to a specific numbered audit run (e.g. C09's run 27/32 reproduction), and changing them without checking each one's evidentiary role risks silently invalidating a preserved result — exactly what §1.2's "preserve the original, a repaired strategy gets a new identity" rule forbids. Full triage of that list is open work (tracked below).
- **Specialist-fitting defect and corrupt QBR rows:** named in the recovered audit; not yet re-verified against current code in this pass (`nfl-specialists.js`'s `fitSpecialists`/`ridgeFit` and `nfl-qbr.js`'s sync path were read and look structurally sound — walk-forward fit, train-only scaling, finite-coefficient ridge — but no live database is reachable from this environment to check actual row-level corruption; that check requires running the coverage audit above against the real `data.sqlite`, not this repo alone).

**Immediate next actions**, in order: (1) finish the Stage-1 data-repair triage — run `nflDataConsistencyAudit()` and `runModelWatch()` against the real database and act on what they report, now that both can see 2026; (2) decide the disposition of the other ~28 stale-season defaults one at a time; (3) connect `trial-statistics.js`'s overfitting checks to a real gate (candidate: `audit-registry.js`'s promotion path) instead of leaving them report-only.
