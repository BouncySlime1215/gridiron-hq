...
**Immediate next actions**, in order: (1) fix the confirmed-live `nfl_qbr_weekly` bulk-seed/duplicate-identity problem — both the upsert-key fix (`season,week,player_id`, not team) and finding why weeks 2–18 were ever populated with copied 2025 data; (2) fix the four freshly-confirmed C05/C06/C07/C09 bugs above (timezone hardcode, news-overwrite `ingested_at`, frozen-packet retry, fake-robust pass) and investigate the `pass_yds` regression / `interception` negative skill `runModelWatch()` flagged; (3) Stage 3 — fit an actual team-strength model on `build_football_dataset`'s 7,291-row history and compare it against the existing rating systems in `nfl-ensemble.js`; (4) migrate the seven remaining inline `fold_cutoff` computations in `market_lab.py`/`tree_lab.py`'s `run()` functions onto `dataset.py::fold_cutoff`/`eligible_training_rows` (governance manual §4.4 item 2, still open); (5) re-verify the four historical (`43af933`-snapshot) Codex findings above against live data before acting on any of them; (6) decide the disposition of the other ~28 stale-season defaults one at a time; (7) connect `trial-statistics.js`'s overfitting checks to a real gate (candidate: `audit-registry.js`'s promotion path) instead of leaving them report-only.
