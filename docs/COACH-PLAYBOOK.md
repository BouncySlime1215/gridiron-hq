...
17. `acquisition_source_and_date` per roster asset (draft round / waiver / trade, date): held-out test found recent trade acquisitions are re-traded 2.13x [1.77,2.52] MORE than other assets, not less — the endowment-effect rationale for avoiding them is unsupported; do not use recency-of-trade as a reason to avoid or deprioritize a target. T4 and target selection.
