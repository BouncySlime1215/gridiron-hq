...
| Negotiation profiles | `scripts/build-negotiation-profiles.mjs` | none | — | Per manager: `says_no`, `praise_means`, techniques, calibration, `roster_read` (`really_untouchable`, `overvalues`, …), `best_bait`, `how_to_approach` | **Sonnet 5**, 16000 tokens (:419) | 09-18 | **Written, never read.** The script header claims the counterparty layer "prefers a model read"; no server code reads the table |
