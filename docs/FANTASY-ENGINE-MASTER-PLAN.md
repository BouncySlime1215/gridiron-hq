...
- **2026-09-19 — both open WA blockers closed, the documented suite baseline found to be 86% wrong, and the A3 structural audit at that boundary.** A new cloud session picked up cold from `HANDOFF.md` at `8cf0387`. Shipped `e8a7831` (RED), `982eb46` (GREEN), `de82ee2`, all pushed. What was established, each by a command run here rather than carried forward:
  ...
  - **The four audit questions:**
    ...
    - **(3) Re-architect anything?** One real candidate, filed rather than acted on because it is production behaviour and not this step's file. `startAiBlindReplay()` (`server/services/nfl-ai-replay.js:351`) forks a `detached: true` + `unref()` worker as a side effect of a plain function call, gated only on a key being present. In the suite that was enough to start real background work mid-run and corrupt an unrelated test. The same shape in production means any caller past the key check spawns a detached process that outlives the request and writes to the live DB. The test-side hazard is closed; the design is not. **[WD]**
