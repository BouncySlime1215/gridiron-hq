...
10. **Log continuously — the most important rule (Nick, 2026-09-18, refined after two follow-ups: "make sure to properly log all of ur work constantly" → "the workflows need to be worked out" → "FYI: running out of usage doesn't kill processes or roll back files. Uncommitted edits stay on disk, and at the switch a watcher writes a report and resumes this same chat on my other account in Terminal").** Correcting my own earlier, more pessimistic read of this: it is the SAME chat resumed, not a cold session, and running out of usage does not kill an in-flight process or discard uncommitted work — both survive the switch. The actual discipline this requires: **before launching any workflow, run `~/claude-handoff/usage.sh`.** If it prints "OK to start a workflow," proceed. If it says **TOO CLOSE**, run `~/claude-handoff/handoff-now.sh "<what you were about to start>"`, tell Nick to switch to the Terminal window, and end the turn — do not start the workflow first and hand off after. Everything still gets logged here regardless (git commits and this document remain the record either account reads), but the check itself is the new required step, not optional diligence.
...
**Confirmed by Nick, 2026-09-18, correcting my earlier, more cautious assumption:** running out of usage does not kill a running process or roll back files — uncommitted edits stay on disk — and the switch resumes this SAME chat on a different account via a watcher-written report, not a cold session with no memory. The gate that actually matters:
1. **Before launching any workflow:** run `~/claude-handoff/usage.sh`. "OK to start a workflow" → proceed. **"TOO CLOSE"** → run `~/claude-handoff/handoff-now.sh "<what you were about to start>"`, tell Nick to switch to Terminal, and end the turn — never launch first and hand off after.
2. If a workflow was already running at a handoff: `git log --oneline -20` and `git status --short` are still the fastest way to see exactly where it is — durable, not session-scoped, ground truth regardless of which account is asking. TDD discipline (rule 5) commits RED before GREEN starts, so the worst case in an uncommitted file is one implementation in progress, never a whole item silently gone.
3. A dirty file found this way is either (a) a build genuinely still in progress — check `ps aux` for a live node/tsc process before touching it — or (b) safely resumable, since the process itself survives the account switch per point 1 above; only treat it as orphaned if the process is confirmed gone.
