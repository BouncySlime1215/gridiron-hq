...
10. **Log continuously — the most important rule (Nick, 2026-09-18, facts verified by a rehearsal on the second account).** FACTS: running out of usage does not kill processes or roll back files — uncommitted edits stay on disk, only API calls stop; a handoff resumes THIS SAME CHAT on the other account in Terminal via a full switch report (last message, last steps, uncommitted git changes, every workflow agent's state, running PIDs, scratchpad/task outputs) — not a cold session. But **a Workflow specifically cannot be resumed from another session** — my own earlier correction overcorrected on this point. If a limit hits mid-run, that run dies; finished agents' results survive in their transcripts, only the unfinished ones get re-run. **The goal is to never start a workflow that can't finish on this account.** Three rules, not optional: (1) before launching ANY workflow, run `~/claude-handoff/usage.sh` — TOO CLOSE means do not launch, run `~/claude-handoff/handoff-now.sh "<what you were about to start>"` instead, tell Nick "handed off, switch to Terminal," and end the turn; the Terminal copy launches it fresh. (2) Keep logging constantly regardless — RED before GREEN, plan edits as they happen, the next step always written down — so the worst case at any moment is one uncommitted implementation, never a whole item. (3) One workflow at a time: while WA runs, WO+WB does not start; once WA's run record appears, run `usage.sh`, then decide.

**Gap found and closed 2026-09-18 (Nick: "did u log all the task ids safley" — no, not until this was found).** Every Workflow launch returns TWO ids, not one: the run id (`wf_...`, for `resumeFromRunId` and the transcript directory) and a separate background **task_id** (needed for `TaskStop`/`TaskOutput` — the only way to actually control a running task from a tool call, not just read it). Only the run id was ever logged tonight (in A5, HANDOFF.md); the task_id was seen once for the scenario-audit workflow and never written down, and WA's was never visible in this context at all (assigned before a compaction). Consequence, proven live: `TaskStop("wf_90ebcd25-088")` failed — that's the run id, not the task_id, and the real one is unrecoverable from inside this session. **Fixed going forward: log BOTH ids the moment a Workflow call returns them, not just the run id.** If a task_id is ever genuinely lost (as WA's now is), the only reliable recovery is the user's own `/tasks` command or the `/workflows` panel, which lists task ids directly — say so plainly rather than guessing at an id.
...
#### A5. Handoff mechanics — before every workflow launch, and what a resumed session should know
**Facts, verified by Nick with a rehearsal on the second account (nsmatta@unc.edu), 2026-09-18 — this is the authoritative version, correcting my own first pass at it:** running out of usage does not kill processes or roll back files; uncommitted edits stay on disk; only API calls stop. A handoff resumes THIS SAME CHAT on the other account in Terminal via a full switch report — last message, last steps, uncommitted git changes, every workflow agent's state (finished / cut off / mid tool call), running jobs by PID, scratchpad and task outputs. **But a Workflow itself cannot be resumed from another session** — if a limit hits mid-run, that run dies; finished agents' results survive in their own transcripts, only the unfinished agents get re-run on relaunch. The goal is to never start a workflow that can't finish on this account, not to rely on a clean resume.
1. **Before launching ANY workflow:** run `~/claude-handoff/usage.sh`. "OK to start a workflow" → proceed. **TOO CLOSE** → do not launch; run `~/claude-handoff/handoff-now.sh "<what you were about to start>"`, tell Nick "handed off, switch to Terminal," and end the turn. The Terminal copy launches the workflow fresh.
2. Keep logging constantly regardless of any of the above (already this session's practice) — RED before GREEN, plan-file edits as they happen, the next step always written down in B1/B3 — so the worst case at any moment is one uncommitted implementation, never a whole item silently gone.
3. **One workflow at a time, strictly:** while WA is running, WO+WB does not start, full stop. Once WA's run record appears (its own completion, not just its latest commit), run `usage.sh` again, then decide whether to launch.
4. If picking this up mid-workflow after a handoff anyway (the run died per the fact above): `git log --oneline -20` and `git status --short` show exactly where it stopped — durable, not session-scoped. A dirty file is either a build genuinely still in progress (check `ps aux` for a live process) or the one uncommitted implementation lost when the run died — read the diff, decide whether to finish it test-first or discard back to the last commit, and say which.
5. Re-read B1 (status/ETA) and B3 (status log, newest first) for what the plan believes is true, then verify the parts that matter against steps 3-4 — the document may not have logged the very last few minutes.
6. **Keep `TASKS.md` (project root) current as work happens, alongside this document, not instead of it (Nick, 2026-09-18, relaying the other session that finished the MCPmarket install).** `## Active` / `## Waiting On` / `## Someday` / `## Done` — the switch report includes Active and Waiting On, so the Terminal copy gets a short pickup list on top of this plan file at handoff. This document stays the full, authoritative record; `TASKS.md` is the fast-read summary a handoff report can quote.
4. Re-read B1 (status/ETA) and B3 (status log, newest first) for what the plan believes is true, then verify the parts that matter against step 2 — the document may not have logged the very last few minutes.
...
#### B3. Status log
- **2026-09-19 — the independent verify returned `issues_unfixed` on this session's own work, and was right.** Recorded here because the finding is about how this session reasons, not only about what it shipped. Every claim below was re-measured before being accepted.
  - **A false claim was published in a commit message, two source files, four docs and a PR comment.** `de82ee2` said the 12 broken-mock tests "made a real request to api.anthropic.com on every run, on every box" and "would have spent real money per run". They could not. With `exports:` silently ignored, `this.fetch` is `{}` and the SDK throws `this.fetch.call is not a function` **before reaching any transport**. Proved with a probe on all four seams writing to a file so forked test children are counted, validated by a control that logs 17 attempts from `offline-guard.test.js` including `api.anthropic.com`: the broken form logs **zero**. All eight places are corrected in place with the original claim quoted.
  - **How the error was made, which is the part worth keeping.** The evidence was in hand and misread. The very first probe of this session returned `cause: this.fetch.call is not a function` — that message *is* the proof no transport was reached — and it was read as a network failure anyway, because the SDK reports a local TypeError in its own transport exactly the way it reports a dead network. Then a real Anthropic `request_id` from `cc12a22` was cited in support; that came from `offline-guard.test.js`'s own deliberately **unmocked** probe, a different file. **One file's evidence was borrowed to describe two others.** The lesson is narrow and checkable: an error message that names a symptom is not evidence of its cause, and "this other commit saw a real request" is not evidence that *these* tests made one.
  - **A second miscount in the same commit.** 11 of the 12, not 12. `page-explain`'s "handles a missing API key gracefully" was fixed by `982eb46`'s credential clearing, not by the mock — `getApiKey()` reads `GRIDIRON_ANTHROPIC_API_KEY` first, so this box's real key defeated that test's own `clearTestKey()`. Two commits were credited with one fix.
  - **A real trap the fix introduced.** `982eb46` made `test/cfbd.test.js:17`'s `const realKey = process.env.CFBD_API_KEY` always `undefined`, and `process.env.X = undefined` stores the **string** `"undefined"`, which is truthy — against a `hasKey()` that is literally `Boolean(process.env.CFBD_API_KEY)`. Harmless today, a trap for the next hook added there. Fixed; it was the only unguarded save/restore of a cleared credential in the suite, and the other three were checked.
  - **Two gaps in the credential surface.** `ANTHROPIC_AUTH_TOKEN` is read by the SDK straight from the environment, so the repo-wide grep that built the list could never have found it — added. And `getApiKey()` has a **third** source the guard does not touch at all: `app_settings.anthropic_api_key` in the database. `npm test` cannot reach it (fresh temp DB); `npm run test:real` opens the real `server/data.sqlite` and can. The guard now states that limit rather than implying a completeness it does not have.
  - **What the verify confirmed, so it is not lost among the corrections:** the credential list is correct for the JS surface; deleting those names breaks nothing; the two mock-fixed files make **zero** network calls and their assertions are real (mutation-tested — perturbing the canned response kills 6 of 8 news tests and 2 page-explain tests, and every survivor is content-independent by design); and no other file has the `exports:` bug (48 `mock.module` call sites: 46 `namedExports`, 2 `defaultExport`, 0 anything else).
  - **The standing rule this produces.** A verifier that only confirms is not evidence. Both independent passes this session overturned something the builder had written down and believed — one a false cause, one a false "pre-existing". A stage verified by the session that built it should not be recorded at the same confidence as one that was not; this entry is the second time that has been demonstrated on this branch in two days.
- **2026-09-19 — both open WA blockers closed, the documented suite baseline found to be 86% wrong, and the A3 structural audit at that boundary.** A new cloud session picked up cold from `HANDOFF.md` at `8cf0387`. Shipped `e8a7831` (RED), `982eb46` (GREEN), `de82ee2`, all pushed. What was established, each by a command run here rather than carried forward:
  - **The previous measurement could not have been valid: this box had no `node_modules`.** A fresh clone does not install, and `npm test` still runs — the suite is mostly Node builtins — so it produces a plausible-looking number against a repo with zero dependencies. `npm ci` first, always, before any suite figure is quoted. Two `offline-guard` tests fail `ERR_MODULE_NOT_FOUND` until it is run, which reads exactly like a guard regression and is not one.
  - **Blocker 1, the never-reported offline-guard verdict: sound, 7/7, on better evidence than the fix shipped with.** `5ac597a`'s GREEN was taken where no valid key existed, so "the SDK was blocked" rested on a 401 returning. Here a **valid** key is present and the SDK is still refused before a packet leaves. The guard's one real limitation is legibility, not correctness: the SDK catches its throw and re-wraps it as a generic `Connection error.`, so `grep "offline test guard"` over a suite log returns 0 while the guard works. Only `offline-guard.test.js:1` digs through `err.cause.cause` to see it.
  - **Blocker 2, the unexplained 15th failure: named, caused, and it was never a regression.** `evidence daemon status exposes feed gaps without faking price evidence`, caused by `ODDS_API_KEY` being present in the GridIron HQ environment while the box the 14-baseline was measured on carried no keys. The suite's verdict was a property of the machine, not the commit.
  - **12 of the documented 14 "known failures" were a typo-class bug in two test files, not an environment gap.** `mock.module('node-fetch', { exports: { default: fn } })` — `exports` is not an option `node:test` has. Ignored silently; the mocked default became an empty object; the SDK died on `this.fetch.call is not a function` and reported its own `Connection error.`. `defaultExport:` fixes it: `nfl-news-events` 1/8 -> 8/8, `page-explain` 2/7 -> 7/7, **with no key present**. `docs/CLOUD-MIGRATION.md:266` and `TASKS.md` both told the next session to re-run these on the Mac, where they would have failed identically; both are corrected in place.
  - **The four audit questions:**
    - **(1) Does each planned build still serve the north star?** Yes — no build is made redundant or wrong-headed by any of this. But it changes what "tested" has been worth on this project. Every "suite green except the known failures" claim made since that baseline was written rested on a baseline that was 86% wrong about its own contents and not reproducible across boxes. No shipped *behaviour* is implicated; the confidence attached to it is.
    - **(2) Is the order still right?** Yes. WA integration remains the last WA item and still gates WO+WB. One correction to how the remaining WA work should be read: the earlier Trade Brain stages were verified against a suite whose result depended on which box ran it, so their "N/N pass" figures were sound only because those files happen not to touch a credential. That is luck, not design, and it is now design.
    - **(3) Re-architect anything?** **No — and this entry's first answer to that was wrong, corrected here rather than left standing.** It filed `startAiBlindReplay()`'s `detached: true` + `unref()` fork (`server/services/nfl-ai-replay.js:351`) to **[WD]** as an open design problem, on the strength of what it did to the test run. Re-reading the function's own comment before acting on that: the fork is deliberate and the reasoning is sound — historical ensemble reconstruction is CPU-heavy, so it runs in its own plain-node process that does not inherit `node --watch`, has no HTTP surface, owns its durable SQLite status, and survives a UI reload or a server hot restart. That is correct for a long job a user starts and walks away from. Nothing about production needs changing, and the WD item is withdrawn.

      What is true is narrower and worth keeping: **`startAiBlindReplay()` spawns a job as the documented effect of being called, and a test called it expecting only a throw.** The test was asserting the not-configured branch and, on a box with a key, fell through into the configured one. That is a test-precondition defect, closed by `982eb46`, not a hint about the architecture. Recording the wrong one of those two would have sent someone to re-engineer a working design.
    - **(4) Anything already built now obsolete?** No build. But the "14 known failures" baseline itself is obsolete and has been corrected in both places it was recorded, rather than left to be re-quoted.
  - **Honest limit, unchanged from the entries below.** Still a fresh clone: no `server/data.sqlite`, no chat DB. Everything here is suite- and fixture-level. None of it touches the Mac list in `TASKS.md`, and none of it should be read as progress against real leagues.
  - **Not yet settled at the time of writing:** the authoritative full-suite run on `de82ee2`, and two independent verify agents launched per A3 (one adversarial re-check of all three commits, one root-causing the remaining `prop-clv-free-capture` and `report-cache` groups). Neither had reported. Recording a stage clean before its verifier reports is precisely what left blocker 1 open overnight — do not repeat it.
- **2026-09-19 — both remaining Trade Brain stages built and independently verified; the A3 structural audit at that boundary.** `value-and-acceptance` and `sendable-proposals` are both in, each with an independent verifier run after the build. The four audit questions:
  - **(1) Does each planned build still serve the north star?** Yes, with one gap that is now visible. The Trade Brain can compute a band, rank ideas and write a sendable proposal — and **nothing renders any of it**. `GET /:leagueId/proposals` returns proposals no page reads, exactly as `availability_basis` was served for days with no page reading it (the second defect in tonight's chance-to-play fix). That is B4's job in WO+WB and is correctly sequenced, but until it lands this stage helps Nick zero.
  - **(2) Is the order still right?** Yes. One correction to a claim made at the previous boundary: `sendable-proposals` was said to be blocked on `value-and-acceptance` only by field shape. In fact it was blocked by something neither audit caught — `findTrades` deals had no `id` at all, so the proposals pass was non-functional in production from the moment it was written. Verified by the verifier's own repro (3 route hits, 3 paid calls, 0 proposals) and fixed in `768d05b`. **Neither the build nor the self-verify found it; the independent pass did.**
  - **(3) Re-architect anything?** One real finding, not yet acted on: **the `trade_proposals` budget is not per-league.** `spentTodayUsd` sums every `trade_proposals:*` key, so the $0.50/day default is shared across all five leagues — roughly ten Sonnet calls a day in total for a feature specified as "once per league per day". The service comment and `sendable-proposals.tdd.md` both claimed a per-league cap and were both wrong; corrected in `f886fae`. Whether to raise the default, make the cap genuinely per-league, or accept the shared budget is a decision with a cost attached, so it is filed rather than quietly changed.
  - **(4) Anything already built now obsolete?** No.
  - **The finding that should change how this plan is run.** Across the two stages tonight: the *self*-verify found one defect and pronounced the stage sound. The *independent* verifies found four more in the same file, then got a fabricated player and fabricated numbers past the one component whose entire purpose is to stop exactly that — numbers returned as JSON values were never checked at all, and a surname on its own was free, which is the single form a model actually writes in. A6's "a verifier who tries to break the result" is not a formality on this project, and a stage verified by the session that built it should not be recorded at the same confidence as one that was not.
  - **Unchanged limit:** no database and no API key in this box, so all of it is fixture-verified, the live Sonnet call has never run, and `dbCache`/migration 060 have zero coverage. The Mac list is in `TASKS.md`.
- **2026-09-19 — WA Trade Brain resumed from a second cloud session, and the structural audit A3 requires at a step boundary.** Picked up cold from `HANDOFF.md` + `TASKS.md` per A5. Three agents launched on disjoint files: `build:value-and-acceptance` (the queued next stage), the offline-guard hermeticity defect, and the `silent-failure-hunter` chance-to-play finding. The four audit questions, answered from what this session actually established rather than restated from the last pass:
  - **(1) Does each planned build still serve the north star?** Yes, with one sharpening. The idea→proposal path was mapped end to end this session and four of its claims re-checked against the code directly (not taken from the mapping report): the sort is on `score_signed` (`trade-engine.js:1706`); `POST /:leagueId/pitch` is **retired, answering 410** (`tradelab.js:228`), so D4's "AI pass" has no surviving pitch route to extend and `build:sendable-proposals` is genuinely new surface, not a rewrite; `llm-budget.js:43` **already ships `trade_proposals: 0.50`** with a `trade_proposals:league-<id>` key convention. **Corrected 2026-09-20: this is LIVE, not dead code.** The chain, re-verified in the tree rather than taken from this file: `routes/trades.js:34` imports `proposalsFor` and `liveCaller`, `:864-865` passes `liveCaller(callClaude)` and `dbCache(lg.id)` into `proposalsFor`, `trade-proposals.js:515` sets `feature: trade_proposals:league-${leagueId}`, and the client renders it (the `proposals` tab in `TradeBrain.tsx`). The original line number was wrong too — it is :43, not :39. **A phrase like "currently dead", "not yet wired" or "no production consumer" in a docs file is an assertion about a past tree, and is re-verified before anyone repeats it.**
  - **(2) Is the order still right?** Yes, and the dependency is narrower than assumed. `value-and-acceptance` edits the idea object in `trade-engine.js:1655-1701` and the score at :1651; `sendable-proposals` should **consume** the idea object and stay out of `trade-engine.js` entirely. That makes them sequential on the field shape only, not on the file.
  - **(3) Re-architect anything?** One correction, found by applying rule 4 (existing systems first) rather than accepting a "this is new work" conclusion. A persisted per-league-day cache for the proposals call looked like new work because `compute-cache.js` is an in-memory `Map` and **migration `057_ai_usage_cost_and_cache` is misleadingly named** — its "cache" is prompt-cache *token accounting* (`cache_read_input_tokens`), not a response store. But a persisted LLM cache pattern **already exists**: `nfl_news_event_extraction_cache` (migration 019, read/written at `nfl-news-events.js:78,83`), keyed `(content_hash, extractor_version)`. Follow that pattern — content-keyed with a version, so an unchanged slate does not re-spend the budget and a changed prompt invalidates by construction — rather than naive day-keying or a second invented cache.
  - **(4) Anything already built now obsolete?** Nothing new. The retired `/pitch` route was already disposed of on 2026-09-18.
  - **The honest limit on everything landing tonight.** A cloud box is a fresh clone: there is no `server/data.sqlite` and no chat DB in it. Every result from this session is **fixture-verified only**. The stages before this one were checked against real leagues on the Mac (the Chase refusal was live-verified); nothing here has that, and the specific items needing the Mac are listed in `TASKS.md`. Do not promote any of tonight's work past "tested" on this evidence.
- **2026-09-18 17:0x — session paused here, safely, at Nick's request ("can u safley log where we are rn and stop the work").** Verified clean before stopping: `git status --short` shows only this session's own untracked `TASKS.md`/`HANDOFF.md` (both intentional, not owned by WA); no file this session edited is mid-change; `usage.sh` reads desktop weekly 96%, TOO CLOSE (5-hour 16%, fine). **Not stopped: WA itself** (`wf_90ebcd25-088`) — Nick's own already-running background workflow, independent of this pause, left running on purpose; stopping it was not asked for and would be destructive. Current WA position: Trade Brain sequence at `tactics-and-packages` — built (`ffe97c9` GREEN), then two follow-up fix commits landed (`9cee38a` fixed 3 real bugs found on real data: the ablation measurement, a draft-skewed active-hours read, a QB-biased sneak-in rule; `f92bb5f` recorded evidence) — a `verify:tactics-and-packages` agent is now active. That item's own scope includes building "the edge test" enforcement the master plan already specifies (line 245) and nothing before this session enforced — **plausibly relevant to bug #1 below, not confirmed either way; check the verify result before assuming it's resolved.**
  - **Two bugs from valuation-map/trade-engine-correctness's own verifiers remain OPEN as of this pause** (full detail: `wf_90ebcd25-088` journal, `grep issues_unfixed`; summarized in `HANDOFF.md`): (1) an edge-test violation — one surfaced trade scored positive only via the perception multiplier while it's -0.48 ppg for Nick on real numbers, violating the plan's own non-negotiable rule; (2) `offerFor`/`offerForMany` refuse a genuinely good horizon-weighted upgrade (Ja'Marr Chase, live-verified) because their ceiling gate is still weekly-only while the ladder below it isn't. Neither is confirmed fixed by tactics-and-packages's commits above — check `wf_90ebcd25-088`'s next verify result first.
  - **On resuming:** re-run `usage.sh` before anything (workflow launches specifically), re-check whether `tactics-and-packages`'s verify closed either open bug, and continue per A5's cold-start procedure.
...
- **silent-failure-hunter** — *high* severity: the validated chance-to-play role layer silently isn't running in production, a bare `catch` turns any read error into "treat as absent" with no log or warning anywhere. Live effect measured: healthy starters like Jayden Daniels show "57% likely to play, check before kickoff" with zero real injury concern.
  - **Re-investigated 2026-09-19 (cloud session), verdict PARTLY FIXED, now closed in code — `docs/tdd/availability-honest-degradation.tdd.md`.** The original bare catch is gone (`review-fixes-2` finding 1, `0a657f6`): a missing fit table is a named basis, warned once, and any other read error throws. Two holes of the same shape were still live and are fixed here (RED `f7ee045`, GREEN `78811b1`, 7/7): (1) `contingency.js#liveEspnStatuses` still read the `leagues` table inside a bare `catch { return null; }`, and since that table is created by the legacy schema migration the catch could only fire on a real fault — when it did, the whole ESPN designation layer vanished silently and an ESPN INJURY_RESERVE starter went 0.006 → 0.953 on the fixture, startable, with `availabilityBasis()` still reporting `role`; (2) `availability_basis` was served to Start/Sit "so the page can say so" and **no page ever read it**, so a healthy starter's pooled 57% was printed identically to the validated number. Start/Sit now serves and renders `availability_note` — the inert layer with its reason, effect and fix, the `counterparty-pricing.js` shape. No served number moves. **Whether the layer is live is still a DB fact, not a code fact: the fit + restart (`play-chance-live.tdd.md` §6) has not run.**
