...
| "Serves ~100 s, dark ~60 s, starts ~160 s apart" | **WITHDRAWN by the same author, for the same reason.** Three of the rows it rested on are not in the surviving probe output and the rest are a few seconds out. Superseded by the row below | scheduler, withdrawn |
| Per-life figures, recomputed from the probe output | **Measured**, six lives in the 22:16:24Z-22:32:40Z window: last `uptime_s` 93, 95, 104, 86, 95, 85; consecutive boots 167, 174, 169 and 166 s apart; 73, 70, 83 and 71 s dark between a life's last answer and the next life's boot. The window's first interval, 253 s, is excluded because its dark stretch could hide a whole life. Boot is read timestamp minus that read's own `uptime_s`, which is grace-independent; last-answer-to-next-**answer** was deliberately not published, because it includes the new machine's routing grace and would have become almost entirely grace once #49 takes it to 300 s | scheduler |
| The brake stopped the loop at 13:43Z | **Measured here, against a prediction written down first.** Anchored probe 13:44:51Z-13:53:31Z: 27 reads, all answered, one derived start on every one, `uptime_s` 99 → 619 unbroken, slowest 0.40 s. Passive logs since: 321 reads, 321 answered, zero dark, one process. Against 1,345 reads and 462 dark over the preceding fourteen hours | this thread |
| The brake cannot say **which** of the two candidates blocked | **Structural, and it is a limit rather than a gap.** One `return` unarms both, so a quiet machine is equally consistent with either. Settled only by the unset and the `nfl_model_growth_runs` read | scheduler + this thread |
| The brake still stops everything after the six merge | **Read off the merged tree `6944033`**, not off `main`: return at `scheduler.js:1927`, every timer below it at :1940, :1964, :1968, :2015, :2016. Above it, `reapAbandonedRuns()` (one parameterised SELECT, one parameterised UPDATE each, no timer, no network) and `onBootComplete?.()` (sets a flag) | coordinator + this thread |
| `main` is 2,950 / 2,909 / 0 / 41 | **Measured** at `791b131`, tree `1b2341a` identical before and after the run | scheduler |
| The merged six are 2,986 / 2,945 / 0 / 41 | **Measured here**, isolated: own worktree, own `npm ci`, tree `6944033` identical before and after, `node_modules` mtime unchanged across the window, clean working tree at the end. Reproduced independently by two threads. An earlier run of this same tree read 2,978 with 2 failures and was **thrown away**, not published — a checkout during the run deleted the stack's own new test files | this thread |
| Per-branch: 2,956 / 2,966 / 2,975 / 2,980 | **Measured**, each at the head its PR points at, each in its own worktree, `npm test` exit 0. Confirmed disjoint by diffing passing-test **names** between consecutive runs: +10, +9, +5, **none lost at any step** — losses being the symptom a disturbed run produces | scheduler |
