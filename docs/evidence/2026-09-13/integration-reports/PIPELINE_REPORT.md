...
> **STALE, corrected 2026-09-19.** `test/nfl-execution-pipeline.test.js` now passes **11/11**, including this test, and it is not clock-coupled. The commit this entry cites as provenance, **`53c408e`, does not exist on any ref in this repository**. The 3 failures later attributed to it were in a different module and table (`nfl-prop-clv.js` / `nfl_prop_quote_snapshots`, not `nfl-execution-clv.js` / `nfl_quote_tape`) and were a time bomb that detonated on 2026-09-17, now fixed in `b0c6e4f`. Do not cite this paragraph as evidence that anything is pre-existing.
