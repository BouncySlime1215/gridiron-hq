...
| D24 | Weekly fantasy scores per league-season are held nowhere, though `view=mMatchup` is requested and the payload persisted | `routes/leagues.js:125`, `:160` | **Open** — unblocks fitting four of feature-audit's thresholds. **Split by the one-editor rule**: `routes/leagues.js` belongs to Google sign-in, so the parse is a new service the fantasy plan writes and a two-line call Google sign-in adds | fantasy plan + Google sign-in |
