...
| F021 | 2026-09-23 | S-03 `a0917685` | fantasy | other | Reproduction of F018-F020 on the final code tree | same | identical output | -0.136 | -0.1915 | -0.0789 | 90 |  | - | n.a. | `docs/evidence/2026-09-22/weekly-construction-walk-forward.md:8` | a re-run spends the week again, so it is a row |
| F022 | 2026-09-22 | BLEND-01 `45f17e38` | fantasy | other | ESPN's weekly projection alone (the tournament's winner) orders the start/sit pairs our served number poses better than our number on 2026 week 2: BLEND-01's rule-5 check | pair accuracy, ESPN − ours, 153 rostered decision rows, 2,778 pairs where ours projects both ≥ 4 (points per decision +1.39 [−0.52, +3.16] on 807 disagreements) | holds (both point estimates > 0); MDE80 0.079 / 2.78 pts | +0.052 | -0.0022 | +0.102 | 90 |  | + | no (held: serving holds waiver_ungraded, forward_unconfirmed; ours is served) | `docs/evidence/2026-09-22/weekly-blend-tournament-output.json` `forward.primary` | local copy, not production; one week: an anecdote for size under rule (e); not blind (read before the pre-registration), so it cannot lift forward_unconfirmed |
| F023 | 2026-09-22 | BLEND-01 `45f17e38` | fantasy | other | Same, with ESPN's archived value for every player (report-only) | pair accuracy, 344 rows, 4,314 pairs (points per decision +1.97 [+0.37, +3.60]) | ESPN ahead | +0.0649 | +0.0219 | +0.1097 | 90 |  | + | n.a. | `docs/evidence/2026-09-22/weekly-blend-tournament-output.json` `forward.archive` | local copy, not production |
| F024 | 2026-09-22 | BLEND-01 `45f17e38` | fantasy | other | Same, with ESPN's Thursday capture, 3 h after our snapshot (report-only) | pair accuracy, 342 rows, 4,314 pairs (points per decision +0.69 [−1.02, +2.37]) | not distinguishable | +0.0357 | -0.0077 | +0.0815 | 90 |  | + | n.a. | `docs/evidence/2026-09-22/weekly-blend-tournament-output.json` `forward.thursday` | local copy, not production; `espn_player_market_weekly` has no writer in the repo |
| F025 | 2026-09-22 | BLEND-01 `45f17e38` | fantasy | other | Same, with the Thursday snapshot the app served (frozen-2023 ensemble x p) as ours (report-only) | pair accuracy, 153 rows, 1,796 pairs (points per decision +2.94 [+0.37, +5.67]) | not distinguishable | +0.0671 | -0.0005 | +0.1351 | 90 |  | + | n.a. | `docs/evidence/2026-09-22/weekly-blend-tournament-output.json` `forward.snapshot` | local copy, not production |
| F026 | 2026-09-22 | BLEND-01 `ac6ac5b8` | fantasy | other | Reproduction of F022-F025 on the tree that adds the labelled availability diagnostic | same | identical output (every pre-registered section byte-identical) | +0.052 | -0.0022 | +0.102 | 90 |  | + | n.a. | `docs/evidence/2026-09-22/weekly-blend-tournament-output.json` `tree` | a re-run spends the week again, so it is a row |
...
S-03's rows (F018-F021) and BLEND-01's rows (F022-F026) were F001-F009 on their branches and were
renumbered when they merged (F016 and F017 are claimed by open PRs #207 and #215). S-02's forward
look at the same 2026 week 2 (2026-09-22 20:45 UTC,
`docs/evidence/2026-09-22/weekly-construction-grade.md` section 8) predates this ledger and has no
row; it is noted on F018.

**BLEND-01's blind forward weeks (added 2026-09-24, FIX-164-1).** The `forward_unconfirmed`
serving hold (`server/services/weekly-blend.js`) lifts only when 2026 weeks whose first game is
after the pre-registration commit (`7c443485`, 2026-09-22 20:21 -04:00) give a pooled ESPN-minus-ours
pair-accuracy 90% CI lower bound above 0. `scripts/weekly-blend-tournament.mjs --forward-blind`
grades each such week once its games all have scores and writes it to the tournament output's
`forward_blind`; each graded week adds one `F` row here (next free id at the time). Graded so far:
none. On 2026-09-24 (local copy) weeks 1-2 are not blind and week 3 (first game 2026-09-24) has
no final scores yet.
...
`F` rows: whichever merges second renumbers. (S-03 and BLEND-01 merged second and took F018-F026.)
