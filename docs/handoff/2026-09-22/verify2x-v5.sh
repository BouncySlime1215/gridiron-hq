#!/bin/bash
# verify2x-v5.sh, 2026-09-22 18:12Z: same as v4 except RUN 2 is conditional on the worktree moving after RUN 1 (Nick's verify-once rule 15:17Z). v4 is unchanged and stays for reproducibility of today's figures.
...
# Snapshot of the worktree RUN 1 will run in. RUN 2 happens only if RUN 1 moved it.
WT_TREE_BEFORE="$(cd "$WT" && git write-tree)"
WT_STATUS_BEFORE="$(cd "$WT" && git status --porcelain)"
echo "WORKTREE write-tree before RUN 1: $WT_TREE_BEFORE"

for RUN in 1 2; do
  if [ "$RUN" = 2 ]; then
    WT_TREE_AFTER1="$(cd "$WT" && git write-tree)"
    WT_STATUS_AFTER1="$(cd "$WT" && git status --porcelain)"
    if [ "$WT_TREE_AFTER1" = "$WT_TREE_BEFORE" ] && [ "$WT_STATUS_AFTER1" = "$WT_STATUS_BEFORE" ]; then
      echo "RUN 2 SKIPPED: tree unchanged after RUN 1 ($WT_TREE_AFTER1)"
      break
    fi
    echo "RUN 2 REQUIRED: worktree moved after RUN 1 (write-tree $WT_TREE_BEFORE -> $WT_TREE_AFTER1)"
  fi
