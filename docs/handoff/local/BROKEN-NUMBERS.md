...
| R | title odds, trade values, 140 card, every sim number | the sim's weekly points about 45% low (league mean 78 vs actual ~130) and NON-uniform: healthy players docked to their career durability rate (0.56-0.85) | nfl_availability_role_rates empty (the scheduler that runs fit-availability is off locally) + K/D-ST not scored + last-season projections | 9/24 2:25 AM: fit-availability run on the live DB (backup kept): 871 role rows; sim league mean 78.5 -> 97.7 (DB copy, 300 runs). Still open: SIM-KDST (about +14), proj.ros (row A), FEAS-140-ESPN | FIX BUILDING: #314 K/DST + #318 as-of (0.916 out-of-sample); availability fit live |
