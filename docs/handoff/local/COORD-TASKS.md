...
12. After #333 merges: flip E2E B10b from todo to a real test (health dot amber on a warn audit).
13. 04:50 After #333 merges (smoke FIX-11 G3): attention.js#leverage says "best move worth 0.0 pts, next move changed" when the re-run has NO move. Pass hasMove; with no move say "no move clears your sliders" (+ "changed since last refresh"); the top strip hides "rank 1 of 1" when only one league is ranked. Bundle with task 12 (B10b) in one coordinator PR; the produce-plans.mjs 'of: r.of' token (task 8) waits for int4.
14. 04:50 auto-intake v4 holds PRs with pending FIXPR/FIXB jobs; watch that ordered PRs still flow (a stuck FIXPR job would block the order behind it: park or finish it).
