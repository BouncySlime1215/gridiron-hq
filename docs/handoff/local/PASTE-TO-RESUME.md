...
0. **ENGINE-00 spine FIRST** (Nick 3:45 PM: "one integrated system, not separate"): engine_events + engine_state + /api/engine/state + backfill adapters; one writer per field. Every later unit is a stage of the one loop (TRADE-INSANE-RND.md top). `critical:true`, risk normal. Ask the spec file for its row, or write it from the brief.
