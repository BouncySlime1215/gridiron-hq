...
- **R&D (SOLE FOCUS: make the Trade Analyzer insane; brief in `$H/TRADE-INSANE-RND.md`; the lanes in rnd-loop-v2.js already point there):** run `wf/rnd-loop-v2.js` with `{"rounds":1,"start":18}`, the next round number after the last "Round N" in LOOP-LOG.md.
