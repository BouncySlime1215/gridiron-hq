...
- Never remove a worktree whose unit's workflow may still be running, even if it looks finished and is clean (coordinator removed REFRESH-L4's at 06:04 while its original workflow agent was still waiting; cost it a run). Check the workflow journal/transcript mtime first.

## Independent code review (Nick 2026-09-24 19:53: "we aren't double checking the code?")
Every PR built by a cloud Project thread (or any builder that did not already get a separate reviewer) gets an INDEPENDENT code review of its diff for correctness bugs before merge: Opus, high effort, a different agent from the one that wrote it. A confirmed bug blocks the merge until fixed on the PR branch. Order per PR: code review -> local measurement on the snapshot against the pre-registered bar -> merge train. The morning brief reports code-review catches.
Efficiency (Nick 19:54 9/24, "that's gonna burn spend"): review the COMBINED diff of a night's batch once (not one reviewer per PR); effort by risk: HIGH for rule enforcement or served numbers, MEDIUM for flagged-off/refactors, none for docs-only; reviewers read the diff plus only the touched functions.
Accuracy (coordinator call 19:55 9/24): one AI reviewer per batch; a SECOND independent reviewer only for code that enforces Nick's rules or changes a served number (disagreements resolved by running the claimed failing case); deterministic gates beat extra reviewers: verify-warroom invariants after every merge + RULE-FUZZ property tests (random rosters/trades, rules never violated in any mode). A revert in an area raises review effort there.
