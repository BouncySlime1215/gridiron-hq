...
# v8: new ideas, each with the data that makes it true and the test (Nick 1:05 AM ET: "if you tell me, I want it true"). Planning only.
- NX-01 Pre-projection window: ESPN's weekly projection updates on a schedule; the news that will move it arrives earlier. We already archive ESPN's projections at more than one time in the week (BLEND-01 used Thursday and lock snapshots), so the lag between a role-change news item and ESPN's number moving is measurable. Offers sent inside that window trade against his screen's OLD number. Test: median lag from news timestamp to projection change, by news type; acceptance of window offers vs others (after N1).
- NX-02 IR-slot arbitrage: league settings (stored mSettings) say how many IR slots each league has. A manager with an open IR slot can stash an injured player for free; one without must cut him. Sell injured players to open-IR-slot teams at screen price; buy from full-roster teams who must dump. Test: price of injured players by the buyer's open IR slots in the 14,882 trades (Sleeper rosters carry IR).
- NX-03 Bench tells: a manager who benches a player he used to start is cooling on him (2026 lineup snapshots: 92 team-weeks so far, weekly from here). Two benchings in a row = buy-low target at his price, if the usage is intact. Test: do benched-with-intact-usage players outscore their price over the next 4 weeks (Sleeper lineups + usage)?
- NX-04 Drop watch: full bench + bye crunch + a pending claim = a drop is coming. Offer bench junk for the player before he hits waivers, where three teams would chase him. Test: precision of the drop predictor on 2026 weeks; share of predicted drops that were claimed by others within 24h.
- NX-05 Cross-league pricing: Nick manages 5 leagues and the same player has a different price in each (AI-04 by league, plus each league's wire). Sell a player where his price is highest, buy where lowest; the same news hits 5 markets at 5 speeds. Test: price dispersion across leagues for the same player-week.
- NX-06 Chat hype as a leading indicator: mention spikes for a player in the group chat (jev signals per player per day) precede price moves in that league. Sell into the chatter. Test: in league 4, mention spike -> transaction price change over the next 7 days (n shown).
- NX-07 Position runs: when injuries thin one position across a league, its prices spike (a run). Sell depth into the run, buy the position nobody is chasing. Test: per-league per-position price index vs injury count that week in the Sleeper corpus.
- NX-08 Market values we already pull: the app already syncs FantasyCalc values (redraft market values built from real trades; syncFantasyCalc, S-10). They are the closest public thing to "what he thinks it's worth" for managers who use a calculator. Use as one his-screen source (AI-07 decides who uses what). Test: FantasyCalc value vs our AI-04 price; which predicts this league's prices better.
