...
The audited local source is commit **`969d501e5d318f8ff650d7e239d8647f8b75eb84`**, seven commits ahead of the pulled GitHub state. The attached **[folder-map.csv](folder-map.csv)** contains the **761 audited baseline files plus nine implementation additions, 770 tracked files total** after the first repair commit. Baseline rows retain their original source commit; added rows identify `fcf7e1f`. Each path has one disposition/destination. Installed plan/evidence rows remain in place, while original document rows still describe the pending consolidation. This is a complete path inventory, not a claim that every proposed semantic split has already been implemented or independently tested. Rows marked `proposed_owner_verify_import_graph`, `mixed_module_requires_extraction`, or `path_sensitive` require the named review before moving. Reconcile source changes after this commit into the manifest before execution; do not discard new local work.
