...
`GET /api/trades/:leagueId/proposals` (`server/routes/trades.js:777`), and the
ledger's state reaches its response as `outcome_ledger` (:783).
...
- **`proposer_team_id`** is `req.query.team_id`, so it is null when the caller
  does not pass one, even though `findTrades` falls back to the league's own team
  (`trade-engine.js:1518`, `myTeamId ?? lg.my_team_id`).
  It is stored as unknown, not guessed.
...
1. **Well built?** Yes, now. It stores what was already being computed and thrown
   ...
   recorded every sent trade as "not selected".
...
3. **How we know:** no backtest; this is a recorder. RED and GREEN on the unfixed
   and fixed code (§2, §3), and a 50-row mutation sweep with 47 killed, no
   unexpected survivors, one designed survivor, and both controls behaving (§4).
...
- §2: RED/GREEN above. All of `92f31b7d`, `e363b35c`, `88ed3722`, `031e4931`
  and `b2c2d500` are ancestors of the pushed head.
