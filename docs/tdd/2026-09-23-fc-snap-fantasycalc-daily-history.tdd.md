...
## Skeptic round 1 (structure lens, tree b40b8986): two blocking issues fixed

1. **Coach vs Trade Lab on retired prices.** Coach runs its own SELECTs over `dynasty_values` (catalogued at `server/services/coach/catalog.js`; the meaning text is put in the model prompt at `server/services/coach/ask.js:46`). The meaning never mentioned `retired_at`, so Coach could read a retired row's last price (local copy, not production: Kaleb Johnson 195 in t10) while Trade Lab, via `currentMarket`, skips it. Fix (skeptic option a): the `dynasty_values` meaning now says a row with `retired_at` set is no longer listed and tells Coach to filter `WHERE retired_at IS NULL`. Test: `test/coach-catalog.test.js` test 18 asserts the meaning names `retired_at IS NULL` and that the column exists on the live table.
2. **Orphan column `dynasty_value_history.age`.** Written by `syncDynastyValues` (`server/routes/aggregates.js:180-183`) but not read. Fix: `marketHistory` (`server/services/dynasty-value-history.js:87`) now selects `age`, so it reaches `GET /api/trades/:leagueId/market-history/:playerId`. Test: `test/dynasty-value-history.test.js` test 14 asserts the route's history rows carry `age` 25, 25.

- RED `dee24fee`: coach-catalog 17 pass / 1 fail (test 18; actual meaning had no `retired_at`); dynasty-value-history 17 pass / 1 fail (test 14; `age` undefined).
- GREEN `47d3d7ae`: coach-catalog 18/18, dynasty-value-history 18/18, coach-ask 13/13, coach-select 21/21, coach-tools 20/20, coach-route 10/10. Command for each: `SCHEDULER_DISABLED=1 GRIDIRON_DB_PATH=$(mktemp -d)/x.sqlite node --experimental-test-module-mocks --test --test-reporter=tap test/<file>.test.js`.
- Local copy, not production: `dynasty_value_history` has 199 rows per format and 199 non-null `age` in each (`select format_key,count(*),count(age) from dynasty_value_history group by 1`), so the served field is populated. `dynasty_values`: t10 199 live / 16 retired, t8 199 / 15.
- Not covered: the catalog meaning is an instruction to the model, not an enforced filter; a model-written SELECT that ignores it could still read a retired row. Enforcing it (a live-rows view catalogued in place of the raw table, skeptic option b) is a follow-up.
