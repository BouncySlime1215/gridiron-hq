...
(`grep -rn 'entities_json\|newsFor' server client/src`, then, after the skeptic pass, `grep -rn "headline LIKE" server`,
which found the three full-name `LIKE` producers the first grep could not):
...
| Trade Lab outlook `playerOutlook` | `server/services/trade-engine.js:2697-2699` | full-name `LIKE` on headline/body, `ORDER BY date DESC LIMIT 5` | `GET /api/trades/:leagueId/player/:id` (`server/routes/trades.js:978`) -> `TradeLab.tsx:980-990` "Recent news". **Unified in this unit** (`playerNews(id, {limit: 5})`), see below |
| Scout report prompt | `server/routes/edge.js:270-272` | headline `LIKE %name%` OR the player's team abbr, `LIMIT 6` | AI scout-report prompt. **Not unified: named follow-up** (it deliberately mixes team news in, a different concept) |
| Draft assist prompt | `server/services/draft-assist.js:907-909` | full-name `LIKE` on headline/body, `LIMIT 3` | draft-assist AI prompt. **Not unified: named follow-up** |
...
- **Skeptic round** (tests 3 and 7 added):
  - `5e5ee4c3` "test: RED Trade Lab outlook must list the card's stories; pin the Jr. suffix strip (RL-12-3)": on the
    previous HEAD code, `# pass 6 # fail 1` (7, Trade Lab contract). Test 3 (a `"Steelers' Pittsworth limited at practice"`
    story, team PIT, must reach Marlon Pittsworth Jr.'s card) passes on HEAD code and is killed by the suffix-strip mutant M11.
  - `c9f8f5d6` "fix: Trade Lab player outlook reads the one news-attribution producer (RL-12-3)": 7/7 pass.
  - RED re-run of the 7-test file against origin/main's `server/routes`, `client` and `server/services/trade-engine.js`
    (checkout, run, `git checkout HEAD --` the same paths): `# pass 0 # fail 7`.
...
legacy-route-security 6/6, table-read-but-never-created 11/11. `node scripts/wiring-map.mjs --check` exit 0 (again exit 0 at c9f8f5d6).
...
## Numbers (local copy, not production; `.local-db/data.sqlite` backed up 2026-09-23 08:38 ET from `~/gridiron-local/data.sqlite`, tree `7d2cd432`; `server/news` and `server/routes` are identical at HEAD, re-run there after the skeptic pass)
...
which is what the News page linked before this unit). Universe: 222 fantasy-relevant, non-historical players with
...
| Card rows not resolved to him | 63 / 579 (23 via the "Jr./III" token) | 40 / 707, all from the surname rule |
| Empty card while resolved stories exist | 19 | 0 |
| Players whose card and News page disagree (all stories, no limit) | nonzero (control: the 190 / 63 above) | 0 / 222 |

Correction (skeptic pass): the first version of this table printed a 224-player universe and 70 / 586 (25) for the
before side. Those came from a run on `cf5d5563`, whose looser surname rule pulled 2 more players into the universe
(the universe filter depends on the attribution rule). Every number in the table above is now from one run on one
tree (HEAD, whose `server/news` + `server/routes` match `7d2cd432`); output: `news_rows 1506, players 222,
before {resolved_top10 675, missed 190, shown 579, shown_not_resolved 63, shown_suffix_token 23, empty 19},
after {missed 8, shown 707, shown_not_resolved 40, empty 0, disagreements 0}`.

Trade Lab (local copy, HEAD): `SCHEDULER_DISABLED=1 GRIDIRON_DB_PATH=$WT/.local-db/data.sqlite node
docs/tdd/2026-09-23-player-news-one-producer/tradelab.mjs $WT` compares the old `playerOutlook` SQL with the card's top 5.
Universe 272 fantasy-relevant players with a story on either side. Before: 55 players with a different story set
(45 more differ only in order); 44 card top-5 stories missing from Trade Lab; 41 Trade Lab rows not in the card's top 5.
After: 0 by construction (`playerOutlook` calls `playerNews`; contract test 7 fails if it forks, M12 killed). The skeptic
got 45 differing players on a 614-player universe with their own script; the universes differ, the direction agrees.
...
## Mutation sweep (`python3 docs/tdd/2026-09-23-player-news-one-producer/mut.py` from the worktree root, re-run on the skeptic-round tree (after the Trade Lab fix), each mutant restored by `git checkout`, tree clean after)

| Mutant | Result |
|---|---|
| M1 card call site drops the newest row (`players.js`) | killed (tests 1, 3, 4, 5, 7) |
| M2 News desk call site back to raw entities (`news.js`) | killed (contract test 5) |
| M3 first-name preceder check disabled | killed (2) |
| M4 follower check disabled | killed (2) |
| M5 order by id instead of published_at | killed (4, 6) |
| M6 team named in the headline ignored | killed (2, 4, 6) |
| M7 resolved ids ignored | killed (4, 6) |
| M8 `/analyze` call site returns `[]` | killed (6). It survived on `7d2cd432`, before that test existed; it was added to kill it |
| M11 suffix strip disabled (`nameParts` keeps "Jr.") | killed (3). Survived before test 3 existed (skeptic finding) |
| M12 Trade Lab call site back to a full-name `LIKE` (`trade-engine.js`) | killed (contract test 7) |
...
2. **Stats or made up?** Every count above comes from the named command on the local copy at HEAD (`server/news` and
   `server/routes` identical to `7d2cd432`), one run, one universe. The wrong-row counts are a hand census of every surname-rule row (46, then 40). They are not a sample.
3. **How we know:** RED 7/7 fail on origin/main code, then GREEN 7/7. 10 of 10 behaviour mutants killed (call sites included), plus the
   named survivors and the control. The route liveness shows the Brooks surgery story on his card.
4. **Pointed elsewhere?** Card, player page, AI Buy/Sell facts, the News desk and the Trade Lab "Recent news"
   panel now all use one rule. Two prompt-only producers still run a full-name `LIKE`: the scout report
   (`server/routes/edge.js:270-272`, mixes in team news) and draft assist (`server/services/draft-assist.js:907-909`).
   Follow-up: route both through `playerNews` (edge.js keeping its team stories as a separate, labelled list).
...
5. **How it unifies:** "which stories are about player X" used to have two producers that disagreed on 190
   resolved stories and 63 shown rows (plus Trade Lab's own third rule). It now has one producer for every display
   list, with 0 of 222 players disagreeing between card and News page.
...
- **Not covered:**
  ...
  - the scout-report and draft-assist prompt producers (`edge.js:270-272`, `draft-assist.js:907-909`), named follow-up;
