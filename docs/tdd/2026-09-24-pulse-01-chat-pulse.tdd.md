...
RED `ea2508af` "test: PULSE-01 chat pulse labels, people events, replan trigger (RED)" · GREEN follows ·
...
- `scripts/people/pulse.mjs`: one pass + replan request (produce-plans --leagues 4 where present and
  GRIDIRON_WARROOM_ENABLED=1; otherwise a recorded status), and `--grade` against hand labels.
...
At `ea2508af` the test imports `server/services/people/pulse.js`, which does not exist:
