...
### Part 9 — league_draft_picks, the second phantom-table read on this file

Found by the wiring map's phantom-table sweep, 2026-09-22, independently of
Parts 6-8b: `buildManagerArchetypes()` ran two bare `SELECT`s against
`league_draft_picks` with no guard at all — the same failure shape
`league_season_teams` had before `leagueHistoryState()`, in the same file,
just never caught the first time because the sweep that found it did not
exist yet.

**Checked before acting, not assumed.** Both `064_league_history_tables.js`
and `backfill-league-history.mjs` carry a comment saying this table "is owned
by `scripts/collect-league-transactions.mjs`'s sibling collector." That file
exists and runs — `git grep` on `origin/main` — but it creates only
`league_transactions_raw`. Grepping every migration and every script in the
repo for `CREATE TABLE ... league_draft_picks` found nothing. The 1,738 live
rows those comments cite were written by something not in this codebase.
Both comments were wrong; the new doc comment on `LEAGUE_DRAFT_PICKS_SOURCE`
corrects them rather than repeating them.

**RED**, `test/league-draft-picks-absent.test.js`, commit `f8c2853`: 4 tests,
0 pass, 4 fail — `leagueDraftPicksState` did not exist and
`buildManagerArchetypes()` threw `no such table: league_draft_picks` on the
absent case, confirmed first with a throwaway repro script before the test
was written.

**GREEN**, commit `daf579b`: `leagueDraftPicksState()` added, mirroring
`leagueHistoryState()` field for field (`present`/`reason`/`source`).
`buildManagerArchetypes()` checks it once at the top and skips the
`league_draft_picks` reads entirely when absent, rather than letting the
`SELECT` throw. Outcome metrics (all-play, luck, career roll-up) are
unaffected by design — `outcomeRows()` is driven by `luckPanel` and
`league_season_teams` only, never by `league_draft_picks` — so a missing
draft-picks table no longer takes the whole build down with it, which the
old unconditional throw did. The return value gained `draft_data_state`
(`'present'` / `'table_absent'`) and `draft_data_reason`, the same shape
`managerProfile()` already uses for `identity_state`/`identity_reason`.
4/4 new tests green; 41 tests across `archetype-as-of`,
`league-history-absent` and `manager-signals-api` unaffected.

**Mutations, hash-verified before and after every run:**

Unmutated `manager-archetypes.js` = `f01e3ff6c4f82a5` (first 16 hex of the
full sha256, generated by `sha256sum`, not transcribed).

| # | Replaced (verbatim) | With (verbatim) | Test file / title | Result |
|---|---|---|---|---|
| M1 | `if (draftState.present) {` (guard on the two `league_draft_picks` reads) | `if (true) {` | `league-draft-picks-absent.test.js` / "buildManagerArchetypes reports the absence instead of throwing a raw SQLite error" | caught — 3 pass, 1 fail |
| M2 | `'cannot be measured at all. This is "we cannot look", not a claim that nobody drafted.'` | `'cannot be measured at all.'` (disambiguation clause dropped) | same file / "the state of league_draft_picks is readable, and the source names the wrong comment" | caught — 3 pass, 1 fail |
| CTRL-NOOP | `/** Is the table there, right now. Deliberately not cached — see leagueHistoryState(). */` | same text, punctuation-only change (`—` → `(...)`) | whole file | stayed green — 4/4, proves the suite is not failing on collateral damage |

After each mutation the file was restored from a saved copy and re-hashed:
`f01e3ff6c4f82a5` both times, confirmed byte-identical, not merely "tests
pass again." M1 targets the guard itself (a reverted fix would throw again);
M2 targets the same weak-assertion failure mode Parts 6-8b already found
four times in this project — an absence message that says WHICH table but
not THAT the lookup failed, versus a claim about the person — and this
sweep found the test already covered it, catching the mutation on the first
try rather than needing a second pass to add the clause.

Full check on the GREEN commit (`daf579b`), isolated (own checkout, real
`node_modules`, not a worktree): `npm run check` exit 0 — figures reported
separately alongside the tree-hash pins in the thread and to the
coordinator, not duplicated here to avoid two authoritative numbers for one
commit.

---

### Part 10 — a note on the name, added after every Part above was written

Everything above, including Part 2's correction about a claim quoting this
symbol, was written while this file's `manager-archetypes.js` accessor was
named `archetypesBuilt` — every occurrence above is that name, deliberately
left as it read at the time rather than rewritten to match what follows.

`archetypesBuilt` was renamed to `archetypeEvidenceBuilt`, commit `abc60e7`,
because a second, unrelated function of the identical name shipped in
`server/services/manager-signals.js:610` (a Trade Brain file, on an unmerged
branch) and produced a false "already wired" read of this one — same name,
two different files, two different functions, one confused report. See
`docs/inventory/chat-sync-evidence.md`'s `archetypeEvidenceBuilt` row for
the full account of that collision. Every citation of `archetypesBuilt` in
Parts 1-9 above refers to the function now called `archetypeEvidenceBuilt`
in `server/services/manager-archetypes.js`; nothing in `manager-signals.js`
is renamed or otherwise touched by this note.
