...
`npm run check` on `1e8dddd` — typecheck, lint, suite, build, `start:smoke` —
...
`1e8dddd` is the parent of the commit that writes this paragraph, which is the
closest a report can honestly get: a measurement cannot be quoted inside the
tree it measures, so the figure names the commit it was taken on and the commit
that prints it comes after. That printing commit touches this paragraph and
nothing the check reads — `scripts/lint.mjs` walks `server`, `scripts` and
`test` for `.js`/`.mjs` only, `tsc` covers the client, and the suite, build and
smoke never open `docs/`. The step is named rather than hidden.

The same figures held at `c986b80`, `da8ec48`, `1171d66` and `511eed8`, every
head this report has had; each was re-measured rather than carried forward, and
each differed from its predecessor by wording in this file alone.
