...
# v2 adds two additive blocks: `model_discipline` (plus a `discipline_passed`
# flag on each fold) and `drift_scans` per substrate. The reader in
# server/services/nfl-research-lab.js accepts v1 and v2 alike: the already
# frozen v1 report on disk (a real run, and a negative result) is evidence and
# must keep rendering; it simply has neither block.
