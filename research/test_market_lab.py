...
class MarketLabTests(unittest.TestCase):
    def test_build_dataset_quarantines_bad_games_and_features_instead_of_silently_dropping_them(self):
        ...
        try:
            ...
            # Reason strings now come from research/betting/nfl/dataset.py's
            # canonical quarantine vocabulary (governance manual section 4.4,
            # item 3: the three previously-independent `dropped` Counters were
            # meant to collapse onto the ONE shared quarantine dict once this
            # lab migrated onto it) rather than this lab's own former names.
