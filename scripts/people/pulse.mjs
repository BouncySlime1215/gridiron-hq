 ...
 *      same lock as the refresh loop's warroom_plans step, only when the War Room flag is on (warroom-flag.js).
 ...
 */
export async function requestReplan(leagueId, { env = process.env, root = ROOT, launch = null, files = null, flag = null } = {}) {
  if (!fs.existsSync(path.join(root, PLANNER))) return { status: 'no_planner', detail: `${PLANNER} is not on this tree` };
  // FIX-316-1: the one War Room flag reader (warroom-flag.js: its own switch or preview mode), injectable for tests.
  const wr = flag ? flag() : (await import('../../server/services/warroom-flag.js')).warRoomFlag();
  if (!wr.enabled) return { status: 'warroom_disabled', detail: 'the War Room flag is off (server/services/warroom-flag.js)' };
...
export async function runPulse({ leagueId = DEFAULT_LEAGUE, env = process.env, now = new Date(), log = console.log,
  ...
  try {
    ...
    // FIX-316-3: Nick's block from the one reader; his unreachable / not-trading managers never replan.
