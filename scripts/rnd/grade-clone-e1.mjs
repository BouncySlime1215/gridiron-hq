...
/* ------------------------------------------------------------- offers */
// Terms come from the raw proposal's items_json, which the collector's upsert can blank once ESPN
// resolves the offer (counted as no_terms). #288 also read trade_proposal_snapshots first; that
// table (#247, migration 084) is not on main and nothing writes it, so this grade does not read it.
...
let decided = 0;
const offers = [];
// Every proposal we know of: a raw proposal row, or only a decision pointing at one.
