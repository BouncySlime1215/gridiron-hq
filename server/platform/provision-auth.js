...
if (!subject || !Number.isInteger(leagueId) || !['member', 'commissioner'].includes(role)) {
  console.error('usage: --subject <stable-id> --league-id <id> [--role member|commissioner] [--draft-team draftId:slot]');
  process.exitCode = 2;
} else {
  await runMigrations();
  ...
  console.log(JSON.stringify({ user_id: userId, league_id: leagueId, role, bearer_token: token }));
}
