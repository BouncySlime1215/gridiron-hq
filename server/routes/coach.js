...
/**
 * NUMBERS-PEOPLE "Ask Coach about this": the item goes into the thread's focus
 * (ids only: move_id, partner, players), then the drawer opens on it.
 */
r.post('/thread/:leagueId/focus', requireAuthenticated, (req, res, next) => {
  try {
    const leagueId = leagueParam(req);
    assertLeagueMember(req.auth.userId, leagueId);
    const b = req.body ?? {};
    const t = setThreadFocus(req.auth.userId, leagueId, { move_id: b.move_id ?? null, partner: b.partner ?? null, players: b.players ?? [] });
    res.json({ thread: { id: t.id, focus: t.focus } });
  } catch (e) { next(e); }
});
