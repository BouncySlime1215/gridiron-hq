/**
 * FEAS-140 + north-star row 9: the "X projected points a week" card as a side panel (pure).
 *
 * Nick asked for the points objective on every league. When a league is planned on
 * title (or playoff, or get-player) odds, this card still answers the points
 * question next to it: how likely he reaches X projected points a week, by which
 * week, what the path costs in players and in title odds, and which byes and
 * injuries drag the lineup under X. X is the league's `side_points_per_week`
 * (objectives file), default 140.
 *
 * It re-uses objectives.js#pointsFeasibility on the same simulated world the plans
 * are scored in; the only new number is the title-odds cost of each option: the best
 * plan's expected title gain minus the option's (both from the same rescores).
 *
 * Flag: GRIDIRON_POINTS_FEASIBILITY ('1' on, '0' off). Unset, it follows the local
 * preview switch (preview-mode.js#previewUnconfirmed), so it is default-off in
 * production and on under the preview switch. A league whose objective
 * is already points keeps the main card; this returns null for it.
 *
 * FEAS-140-ESPN: the target is on ESPN's scale, and the sim runs ~45% low (scale-140.md).
 * Given an `espn` context (espn-lineup.js), status / by_when / first_week_at_target compare
 * ESPN's best projected 10-man lineup per week with the target; a plan uses its post-trade
 * roster's ESPN lineup (espn.forPlan). The sim gives only the spread:
 *   p_hit = P(espn_mean + (sim_run - sim_mean) x espn_mean / sim_mean >= target)
 * and a plan the ESPN context cannot price carries no probability. Without `espn` the card
 * is the incumbent sim-scale report, unchanged (no scale field) until the producer wires it.
 */
import { pointsFeasibility, DEFAULT_SIDE_POINTS } from './objectives.js';
import { PREVIEW_ENV, previewUnconfirmed, previewFields } from '../preview-mode.js';
import { ESPN_SCALE, ESPN_SCALE_LABEL } from './espn-lineup.js';

export const POINTS_FEASIBILITY_ENV = 'GRIDIRON_POINTS_FEASIBILITY';
export const DEFAULT_POINTS_TARGET = DEFAULT_SIDE_POINTS;
export const SIDE_OPTIONS = 5;
const PREVIEW_REASON = 'FEAS-140 points side panel: default-off until Nick confirms the card on the target league';

const mean = xs => (xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : null);

/** Per-run season average of weeklySummary input over `weeks` (runs aligned by index), or null. */
function simRuns(simWeeks, weeks) {
  const rows = (simWeeks ?? []).filter(w => weeks.includes(w.week) && Array.isArray(w.samples) && w.samples.length);
  if (!rows.length) return null;
  const n = Math.min(...rows.map(w => w.samples.length));
  return Array.from({ length: n }, (_, i) => mean(rows.map(w => Number(w.samples[i]) || 0)));
}

/**
 * The sim's spread on ESPN's level: P(espnMean + (run - simMean) x espnMean / simMean >= target).
 * Null when there is no sim spread to borrow (then the card shows no probability).
 */
export function espnHitProbability(espnMean, runs, target) {
  if (!Number.isFinite(espnMean) || !Array.isArray(runs) || !runs.length) return null;
  const simMean = mean(runs);
  if (!(simMean > 0)) return null;
  const k = espnMean / simMean;
  return runs.filter(r => espnMean + (r - simMean) * k >= target).length / runs.length;
}

/**
 * pointsFeasibility on ESPN's scale. espn: { now: view, forPlan?: plan -> view | null } where a
 * view is espn-lineup.js#espnLineupContext().forTeam(...) (weeks(list), ros_per_game).
 */
export function espnPointsFeasibility({ target, nowWeeks, espn, options = [], roster = [], currentWeek }) {
  const weeks = [...new Set(nowWeeks.map(w => w.week))].sort((a, b) => a - b);
  const baseWeeks = espn.now.weeks(weeks);
  const baseMean = mean(baseWeeks.map(w => w.total));
  const baseP = espnHitProbability(baseMean, simRuns(nowWeeks, weeks), target);
  const opts = options.map(o => {
    const view = espn.forPlan?.(o) ?? null;
    const after = weeks.filter(w => w >= (o.arrive_week ?? -Infinity));
    const common = { label: o.label, p_complete: o.p_complete, arrive_week: o.arrive_week, steps: o.steps,
      cost_players: o.give.length, give: o.give };
    if (!view || !after.length) {
      return { ...common, priced: false, season_mean_after: null, p_average_hits_if_done: null, p_average_hits: null,
        first_week_at_target: null };
    }
    const aw = view.weeks(after);
    const m = mean(aw.map(w => w.total));
    const ifDone = espnHitProbability(m, simRuns(o.weeks, after), target);
    return { ...common, priced: true, season_mean_after: m, ros_per_game_after: view.ros_per_game,
      p_average_hits_if_done: ifDone,
      p_average_hits: ifDone == null || baseP == null ? null : o.p_complete * ifDone + (1 - o.p_complete) * baseP,
      first_week_at_target: aw.find(w => w.total >= target)?.week ?? null };
  }).sort((a, b) => (b.p_average_hits ?? -1) - (a.p_average_hits ?? -1)
    || (b.season_mean_after ?? -Infinity) - (a.season_mean_after ?? -Infinity));
  const warnings = [];
  for (const w of baseWeeks) {
    if (w.total >= target) continue;
    const onBye = roster.filter(p => p.starter && p.bye === w.week);
    if (onBye.length) warnings.push({ kind: 'bye', week: w.week, players: onBye.map(p => p.id),
      text: `Week ${w.week}: ${onBye.map(p => p.name).join(', ')} on bye; ESPN lineup projects ${w.total.toFixed(1)} vs ${target}.` });
  }
  for (const p of roster.filter(q => q.starter && q.injury)) {
    warnings.push({ kind: 'injury', players: [p.id], text: `${p.name} carries an injury flag; his weeks may be zeros.` });
  }
  const best = opts.find(o => o.priced) ?? null;
  const status = baseMean >= target ? 'on_track' : best && best.season_mean_after >= target ? 'reachable' : 'out_of_reach';
  const firstNow = baseWeeks.find(w => w.total >= target)?.week ?? null;
  return {
    target, status, scale: ESPN_SCALE, scale_label: ESPN_SCALE_LABEL,
    now: { season_mean: baseMean, ros_per_game: espn.now.ros_per_game, p_average_hits: baseP,
      per_week: baseWeeks.map(w => ({ week: w.week, mean: w.total, posted: w.posted })) },
    how_likely: best && best.p_average_hits != null ? Math.max(best.p_average_hits, baseP ?? 0) : baseP,
    by_when: status === 'on_track' ? firstNow ?? currentWeek : best?.first_week_at_target ?? null,
    at_what_cost: best ? { players: best.cost_players, give: best.give, steps: best.steps } : null,
    options: opts.slice(0, 5), warnings,
  };
}

/** { on, preview } for the side panel. An explicit flag wins over the preview switch. */
export function pointsFeasibilityFlag(env = process.env) {
  const v = env[POINTS_FEASIBILITY_ENV];
  if (v === '1') return { on: true, preview: false };
  if (v === '0') return { on: false, preview: false };
  const preview = env === process.env ? previewUnconfirmed() : env[PREVIEW_ENV] === '1';
  return { on: preview, preview };
}

/** The side-panel target: the league's configured number, else 140. */
export function sidePointsTarget(objective = {}) {
  const t = Number(objective.side_points_per_week);
  return t > 0 ? t : DEFAULT_POINTS_TARGET;
}

/**
 * objective:  normalised objective (objectives.js#normaliseObjective)
 * nowWeeks:   weeklySummary input for today's roster ([{ week, samples }]) or null
 * plans:      ranked plans on the league objective, best first:
 *             [{ label?, expected, p_complete, arrive_week, give: ids, steps: n, weeks: [{ week, samples }] }]
 * roster:     [{ id, name, bye, injury, starter }]
 * espn:       optional { now, forPlan } (espnPointsFeasibility); absent -> the incumbent sim-scale report
 * Returns null when the flag is off, the league is already on the points objective,
 * or the world has no weekly lineup points. Otherwise the pointsFeasibility report
 * plus p_reach / arrive_week / cost (players and title odds) and the flag fields.
 */
export function sidePanelFeasibility({ objective, nowWeeks, plans = [], roster = [], currentWeek, espn = null, env = process.env }) {
  const flag = pointsFeasibilityFlag(env);
  if (!flag.on || !objective || objective.kind === 'points' || !Array.isArray(nowWeeks) || !nowWeeks.length) return null;
  const target = sidePointsTarget(objective);
  const opts = plans.filter(p => Array.isArray(p?.weeks) && p.weeks.length).slice(0, SIDE_OPTIONS)
    .map((p, j) => ({ ...p, label: p.label ?? `plan ${j + 1}` }));
  const bestExpected = opts.length ? Math.max(...opts.map(p => Number(p.expected) || 0)) : 0;
  const titleCost = new Map(opts.map(p => [p.label, Math.max(0, bestExpected - (Number(p.expected) || 0))]));
  const optIn = opts.map(p => ({ ...p, give: p.give ?? [] }));
  const r = espn?.now
    ? espnPointsFeasibility({ target, nowWeeks, espn, currentWeek, roster, options: optIn })
    : pointsFeasibility({ target, now: nowWeeks, currentWeek, roster,
      options: optIn.map(p => ({ label: p.label, weeks: p.weeks, p_complete: p.p_complete, arrive_week: p.arrive_week,
        give: p.give, steps: p.steps })) });
  const options = r.options.map(o => ({ ...o, title_odds_cost: titleCost.get(o.label) ?? null }));
  const best = options.find(o => o.priced !== false) ?? null;
  const onTrack = r.status === 'on_track';
  return {
    kind: 'points', side_panel: true, league_objective: objective.kind, ...r, options,
    p_reach: r.how_likely,
    arrive_week: r.by_when,
    cost: onTrack || !best ? { players: 0, give: [], steps: 0, title_odds: 0, basis: onTrack ? 'already on track' : 'no plan to price' }
      : { players: best.cost_players, give: best.give, steps: best.steps, title_odds: best.title_odds_cost,
        basis: 'best plan on the league objective minus this option, same rescores' },
    warnings_count: { bye: r.warnings.filter(w => w.kind === 'bye').length, injury: r.warnings.filter(w => w.kind === 'injury').length },
    ...(flag.preview ? previewFields(PREVIEW_REASON) : {}),
  };
}
