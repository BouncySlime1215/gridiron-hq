 ...
 *                  prices him) and adapter.untouchable, on every rung
 *   never get      NEVER_GET (Chris Olave 290: sold 9/17), on every rung
...
export const DROP_REASONS = Object.freeze(['not_a_ladder', 'never_get', 'gives_untouchable', 'overpay',
  'final_unscored', 'final_below_floor', 'all_in_rungs_p_guess', 'duplicate']);
 ...
 * opts: { mode, scoreOf, players (Map id -> { value }), untouchable (Set), maxOverpay, n }
...
export function ladderCards(plans, { mode = 'balanced', scoreOf = null, players = new Map(), untouchable = new Set(),
  ...
  const never = new Set([...NEVER_GIVE, ...[...(untouchable ?? [])].map(String)]);
  ...
  const onNo = (p, i, rungs) => {
    ...
    const alt = kept.find(q => q !== p && q.steps.length > i && prefixOf(q, i) === pre && dealKey(q.steps[i]) !== dealKey(p.steps[i]));
    if (alt) {
      const s = alt.steps[i];
      return { kind: 'backup', partner: String(s.team), give: s.give.map(String), get: s.get.map(String),
        expected: pathExpectation(alt.steps).expected };
    }
  ...
  return { mode: m, floor: FINAL_FLOOR, rank_basis: 'p_guess', considered: plans.length, dropped_by_reason: dropped, cards };
}
...
export function ladderSection(l, { names = {}, unit = 'title_odds' } = {}) {
    ...
    .map(c => ({ target: c.target, owner: c.owner, climb: c.climb, rank_basis: c.rank_basis,
      rungs: c.rungs.map(r => ({ partner: r.partner, give: r.give, get: r.get, get_tier: r.get_tier,
        ...
        on_no: r.on_no.kind === 'backup'
          ? { kind: 'backup', partner: r.on_no.partner, give: r.on_no.give, get: r.on_no.get, expected: d(r.on_no.expected) }
  ...
  return { status: 'ok', source: 'plan.path', guess: true,
    value: { mode: l.mode, floor: l.floor, rank_basis: l.rank_basis, considered: l.considered, dropped_by_reason: l.dropped_by_reason, cards } };
}
