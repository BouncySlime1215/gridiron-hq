...
/** Rows for one league's plan result ({ league, week, deck }); [] when no step carries both estimators. */
export function rbShadowRows(res) {
  const rows = [];
  for (const card of res?.deck ?? []) {
    ...
    plan.steps.forEach((st, i) => {
      ...
      rows.push({
        week: res.week ?? null, move_id: id, step: i + 1,
...
/** { rows, sign_flips, within_2se_share, passes } for the Tuesday review rule. */
