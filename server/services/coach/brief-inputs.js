 ...
 */
export function readStatements(chat, { names, since, until, credibility = new Map() }) {
  ...
  const raw = chat.prepare(`SELECT s.name AS speaker, s.question AS question, s.probability AS p, m.ts_utc AS at
  ...
  return { status: 'ok', rows, as_of: until };
}
 ...
 */
export function readReplies(db, { leagueId, me, since, until, partnerOf = () => null }) {
  const rows = [];
  const read = [];
  if (tableExists(db, 'trade_outcomes')) {
    read.push('trade_outcomes');
    for (const r of db.prepare(`SELECT counterparty_team_id AS team, status, resolved_at AS at
  ...
  if (tableExists(db, 'warroom_requests')) {
    ...
    for (const r of db.prepare(`SELECT id, payload, created_at AS at FROM warroom_requests
      ...
      if (team && rows.some(x => x.via === 'trade_outcomes' && x.team === team && x.reply === p.reply)) continue;
 ...
 */
export function readInjuries(db, { leagueId, me, since, until, watch = [] }) {
  ...
  return { status: 'ok', rows, as_of: until, period: latest.period };
}
