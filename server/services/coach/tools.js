 ...
 * preview mode is on, plus negotiate_reply (negotiator.js, COACH-NEGOTIATE)
 * when GRIDIRON_COACH_NEGOTIATE or preview mode is on. Read per call, so a flag
...
export function toolDefinitions({ warRoom = false } = {}) {
  const tools = warRoom ? [...activeTools(), ...WARROOM_TOOLS] : activeTools();
...
export function runCoachTool(name, input, { ledger } = {}) {
  ...
  const tool = tools.find(t => t.name === name) ?? WARROOM_TOOLS.find(t => t.name === name);
  if (!tool) {
