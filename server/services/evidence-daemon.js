...
function dueWindows(force = false) {
  ...
  const openHorizon = new Date(Date.now() + 7 * 86400000).toISOString();
  return rows(`SELECT * FROM evidence_capture_windows
    WHERE due_at <= ? AND event_at >= ? AND (
      status='queued' OR (status='partial' AND COALESCE(last_attempt_at,'') < ?)
    ) AND (horizon <> 'open' OR event_at <= ?) ORDER BY event_at,horizon`, now, new Date(Date.now() - 5 * 60000).toISOString(), retryBefore, openHorizon)
  ...
  try {
    ...
    for (const [date, windows] of mlbGroups) {
      try {
        ...
        const complete = result.odds_available && result.quotes > 0;
...
export function evidenceDaemonStatus() {
  ...
  return {
    running: false, odds_feed: hasKey(), by_status: byStatus, by_horizon: byHorizon, next,
    ...
    alerts: [
      ...
      ...(partial ? [{ severity: 'warn', code: 'partial_windows', message: `${partial} due capture windows have context only and will retry when the odds feed is available.` }] : [])
    ]
  };
}
