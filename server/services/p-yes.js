 ...
 */
export function pYesTableFrom(offers, leagueId, teams = null, { now = Date.now(), mode = 'baseline', firstSight = null } = {}) {
...
export function pYesTable(database, leagueId, teams = null, { now = Date.now(), mode = pYesFlag().mode, env = process.env } = {}) {
  ...
  const table = { ...pYesTableFrom(offers, leagueId, teams, { now, mode, firstSight }), league: String(leagueId), reason: reason ?? null };
