...
export function respondsAdjust(pr, cp, myIds, { baseAnchor }) {
  ...
  let p = pr.p;
  // partners.js#pResponds already applied Nick's flags (basis 'Nick: ...', read from the same reader block):
  // RULINGS 17, the counterpart reads them and never applies them a second time.
  const nickApplied = /^Nick:/.test(pr.basis ?? '');
  if (pr.p > 0 && !/checked out|never trading|not trading|unreachable/.test(pr.basis)) {
