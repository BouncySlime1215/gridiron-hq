 ...
 *
 * SHADOW ONLY (batch D rebase of #288). The clone is a LIVE-BLEND challenger
 * arm: with GRIDIRON_CLONE_V2=1 it rides on the served acceptance as
 * `p_yes_challenger` and is never read by acceptanceBand, p-yes.js or the blend,
 * so the served P(yes), its band and every rule are the same with the flag on
 * or off. Only its own flag switches it on: preview mode does not.
 */
export const CLONE_ENV = 'GRIDIRON_CLONE_V2';
/** The challenger's model id on `p_yes_challenger` (LIVE-BLEND's own arms are baseline and clone). */
export const CLONE_V2_MODEL = 'clone_v2';
...
/** Largest declared P(veto). */
export const VETO_P_CAP = 0.30;

const logit = p => Math.log(p / (1 - p));
const sigmoid = x => 1 / (1 + Math.exp(-x));

/** On only when its own flag is exactly '1'. Read per call. Preview mode never turns it on. */
export function cloneMode(env = process.env) {
  return { on: env[CLONE_ENV] === '1' };
}
...
export function cloneFor({ counterparty = null, pool = null, fit = null, gainPct = null } = {}) {
  ...
  return { p, prior_p: prior.p, n: replies.length, k, history_n: prior.history_n, gain_pct: gainPct,
    price_bound: priceBound, pool: prior.pool, offsets: prior.offsets, fitted: false, reason, why };
}
...
export function vetoFactor(risk) {
  ...
  const p = Math.min(VETO_P_CAP, VETO_P[base.level] ?? VETO_P.unknown);
...
/**
 * The shadow `p_yes_challenger` block for one served acceptance: the clone's P(accept), and
 * P(complete) = P(accept) x (1 - P(veto)) when the league states its veto votes. Never a band, never
 * served: `served: false` says so on every block, and nothing in the served path reads it.
 */
export function challengerOf(clone, veto = null) {
  if (!clone || !Number.isFinite(clone.p)) return null;
  const pVeto = Number.isFinite(veto?.p_veto) ? veto.p_veto : null;
  return {
    model: CLONE_V2_MODEL, served: false, shadow: true, fitted: false,
    p: round(clone.p), prior_p: round(clone.prior_p), n: clone.n, k: clone.k, gain_pct: clone.gain_pct,
    price_bound: clone.price_bound, pool: clone.pool, offsets: clone.offsets, reason: clone.reason, why: clone.why,
    completion: pVeto == null
      ? { p: null, p_veto: null, fitted: false, reason: veto?.reason ?? 'no veto read was supplied' }
      : { p: round(clone.p * (1 - pVeto)), p_veto: pVeto, level: veto.level ?? null, fitted: false, why: veto.why },
  };
}
