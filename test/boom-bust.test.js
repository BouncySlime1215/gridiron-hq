...
// Side-effect import: player_week_usage is created ad-hoc at import time by
// nflverse.js (same "~40 files create tables on import" pattern the rest of
// the suite relies on).
...
function usage(season, week, name, opts = {}) {
  ...
  run(`INSERT INTO player_week_usage (player_id, season, week, position, receptions, receiving_yards, receiving_tds, targets)
       VALUES (?,?,?,?,?,?,?,?)`,
    id, season, week, opts.position ?? 'WR', opts.receptions ?? 0, opts.receiving_yards ?? 0,
...
test('a top-ADP player who actually produces is graded "as expected", not a boom or bust', () => {
...
test('buildBoomBustDataset produces one feature row per matched player, aligned X/y', () => {
  const { X, y, meta, featureNames } = buildBoomBustDataset({ fromSeason: 2022, throughSeason: 2022 });
