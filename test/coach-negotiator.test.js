...
// INT5 (#327 x #325/#298): activeTools() warms the engine worker whenever the tool is
// offered, and int4's navigator and brain-tool tests call it under preview mode. A warm
// worker must never keep a short-lived process alive: the listeners re-ref the worker,
// so it is unref'd after them (before this fix the child below never exited).
test('a warmed negotiator worker does not keep a short-lived process from exiting', async () => {
  const { spawnSync } = await import('node:child_process');
  const toolsUrl = pathToFileURL(path.join(path.dirname(new URL(import.meta.url).pathname), '../server/services/coach/tools.js')).href;
  const script = path.join(temp, 'warm-exit.mjs');
  fs.writeFileSync(script, `process.env.GRIDIRON_PREVIEW_UNCONFIRMED = '1';
const t = await import(${JSON.stringify(toolsUrl)});
if (!t.activeTools().some(x => x.name === 'negotiate_reply')) throw new Error('negotiate_reply not offered');
`);
  const env = { ...process.env, SCHEDULER_DISABLED: '1', GRIDIRON_DB_PATH: path.join(temp, 'warm-exit.sqlite') };
