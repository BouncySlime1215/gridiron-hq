 ...
 * whole chain — and an UNSTABLE one: a shout rate that is entirely different
 * in the second half from the first. A harness worth having passes the first
...
for (const person of PEOPLE) {
  for (let m = 0; m < 30; m++) {
    ...
    // Stable: the same latency in both halves. Unstable: shouting only late.
    const shout = m >= 21;
...
test('too few people is "not enough data", never a pass and never a fail', () => {
  ...
  for (const grade of report.graded) {
    ...
    assert.match(grade.reason, /people|sample/i);
  }
});
