...
// The fixture is the real producer's output (FIX-03): move ids are hashes, so read them from it.
const E0 = PLANS_DOC.leagues[0];
const M1 = E0.next_move.value.move_id;
const M2 = E0.alternatives.value.find(m => m.move_id !== M1).move_id;
const ST = E0.next_move.value.steps[0];
const ME = String(E0.me), PARTNER = String(ST.partner), GIVE = ST.give.map(String), GET = ST.get.map(String);
const P_YES = ST.p_yes.value, DELTA = ST.title_odds_delta.value, SE = ST.title_odds_delta.se, AFTER = ST.title_after.value;
const TITLE_NOW = E0.destination.value.title_now.value;
const BAND = { low: Math.min(0.25, P_YES), high: Math.max(0.5, P_YES) };

function league(id) {
  run(`INSERT INTO leagues (id, platform, league_id, season, name, payload, team_count, my_team_id, current_week)
       VALUES (?, 'espn', ?, ?, ?, ?, 4, ?, 4)`, id, `espn-fix07-${id}`, SEASON, `L${id}`, JSON.stringify({ teams: [] }), ME);
  return row('SELECT * FROM leagues WHERE id = ?', id);
}
for (const id of [FIXTURE_LEAGUE, 11, 12, 13, 14, 15, 16, 17]) league(id);
for (const id of [...new Set([...GIVE, ...GET].map(Number))]) {
  run('INSERT INTO players (id, name, position, espn_id) VALUES (?, ?, ?, ?)', id, `Made Up ${id}`, 'WR', id + 40000);
}

/**
 * The contract fixture, with a p_yes_band on each first step (none when `band` is false), and a made-up
 * opening where the producer priced none (G5b needs an opening to send).
 */
function plans({ band = true } = {}) {
  ...
  for (const e of doc.leagues) {
    for (const m of [e.next_move?.value, ...(e.alternatives?.value ?? [])]) {
      const st = m?.steps?.[0];
      if (!st) continue;
      if (band) st.p_yes_band = { low: Math.min(0.25, st.p_yes.value), high: Math.max(0.5, st.p_yes.value), basis: 'heuristic_unanchored' };
      else delete st.p_yes_band;
      if (st.opening?.status !== 'ok') st.opening = { status: 'ok', source: 'clone.price', value: { give: st.give.slice(0, 1), get: st.get, text: 'made-up opening' } };
    }
  }
...
test('G1b: mode.set until_week reaches the contract as destination.risk_mode.until_week (FIX-03 key)', () => {
  ask(17, 'mode.set', { mode: 'safe', until_week: 6 });
  const inputs = leagueInputs(17);
  assert.equal(inputs.objective.risk_until_week, 6);
  assert.deepEqual(planWith(inputs).destination.value.risk_mode.value, { mode: 'safe', until_week: 6 });
  consumeWith([inputs.consume], () => {});
});

test('the producer takes War Room inputs through buildPlansFile and stamps them with the plans write', () => {
  const src = fs.readFileSync(path.join(HERE, '..', 'scripts', 'campaign', 'produce-plans.mjs'), 'utf8');
  assert.match(src, /leagueInputs, consumed/);
  assert.match(src, /consumeWith\(consumed/);
  assert.doesNotMatch(src, /offers\.jsonl|offerLog/, 'offers.jsonl goes (FIX-07)');
});
...
test('G4: a deck skip stores its card and down-weights that manager', () => {
  const p = plans();
  const skip = ask(FIXTURE_LEAGUE, 'deck.skip', { move_id: M1, reason: 'dont_trust_manager' }, { plans: p });
  assert.equal(skip.payload.card.partner, PARTNER);
  const inputs = leagueInputs(FIXTURE_LEAGUE);
  assert.ok(inputs.weights.manager.get(PARTNER) < 1, 'the skipped manager is down-weighted');
  const cost = ask(FIXTURE_LEAGUE, 'deck.skip', { move_id: M1, reason: 'costs_too_much' }, { plans: p });
  assert.deepEqual(cost.payload.card.get, GET);
  assert.ok(leagueInputs(FIXTURE_LEAGUE).weights.player.get(GET[0]) < 1, '"costs too much" down-weights what the card gets');
});
...
test('G5: "I sent it" writes exactly one trade_outcomes row with sent_at and band; a second tap is already_sent', () => {
  const p = plans();
  const first = ask(FIXTURE_LEAGUE, 'offer.sent', { move_id: M1 }, { plans: p });
  ...
  assert.equal(o.counterparty_team_id, PARTNER);
  assert.equal(o.move_id, M1);
  assert.equal(o.price_band, 'at_point');
  assert.equal(o.model_p_accept_low, BAND.low);
  assert.equal(o.model_p_accept_high, BAND.high);
  assert.equal(o.model_p_accept, P_YES, "the card's p_yes is the midpoint");
  assert.deepEqual(JSON.parse(o.give_json).map(x => x.espn_id).sort(), GIVE.map(g => Number(g) + 40000).sort(), 'ESPN ids ride along for the settle job');

  const second = ask(FIXTURE_LEAGUE, 'offer.sent', { move_id: M1 }, { plans: p });
...
test('G5b: the opening sent is priced below the yes-point', () => {
  const p = plans();
  const r = ask(FIXTURE_LEAGUE, 'offer.sent', { move_id: M1, sent_as: 'opening' }, { plans: p });
...
test('G6: a card with no p_yes_band records no trade_outcomes row and says why', () => {
  ...
  const r = ask(15, 'offer.sent', { move_id: M1 }, { plans: doc });
  ...
  const off = ask(15, 'offer.sent', { move_id: M1 }, { plans: bad });
...
test('G7: retracting "I sent it" takes the sent mark back off', () => {
  ...
  const sent = ask(16, 'offer.sent', { move_id: M2 }, { plans: doc });
...
test('G9: consuming an "I sent it" writes the campaign_steps predicted row', () => {
  ...
  const s = row('SELECT * FROM campaign_steps WHERE league_id = ? AND move_id = ?', FIXTURE_LEAGUE, M1);
  assert.equal(s.predicted_title_odds_gain, DELTA);
  assert.equal(s.predicted_se, SE);
...
test('G10: the shown next move is logged to follow_ledger once, and the war_room surface is served', () => {
  ...
  assert.equal(JSON.parse(logged[0].pick_json).move_id, M1);
  ...
  const deal = `deal:${ME}|${PARTNER}|`;
  assert.equal(pick(deal, 'p_yes'), P_YES);
  assert.equal(pick(deal, 'title_odds_delta'), DELTA);
  assert.equal(pick(deal, 'title_after'), AFTER);
  assert.equal(pick(`team:${ME}`, 'title_now'), TITLE_NOW);
