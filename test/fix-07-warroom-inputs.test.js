...
test('G4: a deck skip stores its card and down-weights that manager', () => {
  const p = plans();
  const skip = ask(FIXTURE_LEAGUE, 'deck.skip', { move_id: M1, reason: 'manager' }, { plans: p });
  ...
  const cost = ask(FIXTURE_LEAGUE, 'deck.skip', { move_id: M1, reason: 'cost' }, { plans: p });
  assert.deepEqual(cost.payload.card.get, GET);
  assert.ok(leagueInputs(FIXTURE_LEAGUE).weights.player.get(GET[0]) < 1, 'a "cost" skip down-weights what the card gets');
});
