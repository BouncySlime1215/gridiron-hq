 ...
 * (server/services/campaign/produce.js#buildPlansFile, the function
 * scripts/campaign/produce-plans.mjs runs) on the made-up league in
 * test/fixtures/campaign-league.mjs. No real data.
 *
 * Four leagues, so the file exercises every branch a consumer reads:
 *   1  title, balanced, Nick's stops (get / sell an untouchable / bye / custom),
 ...
 *   3  points objective, team 4 checked out (a "desperate" catch-up move)
 *   4  go get player 21
 *
...
import { buildPlansFile } from '../../../server/services/campaign/produce.js';
...
export async function makeProducerPlans() {
  const leagues = [
    ...
    { id: 3, load: async () => ({ adapter: leagueOf(3, { managerExtra: { 4: { checked_out: true } } }) }) },
    { id: 4, load: async () => ({ adapter: leagueOf(4) }) },
  ];
  return buildPlansFile(leagues, { generated_at: GENERATED_AT, objectives: OBJECTIVES, clock: () => 0 });
}
