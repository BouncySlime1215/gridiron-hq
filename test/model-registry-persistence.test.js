...
test('latest migration down and re-up are transactional and reproducible', async () => {
  assert.equal(await rollbackMigration('015_espn_durable_sync'), '015_espn_durable_sync');
  assert.equal(await rollbackMigration('014_espn_authoritative_board'), '014_espn_authoritative_board');
  assert.equal(await rollbackMigration('013_espn_live_draft_setup'), '013_espn_live_draft_setup');
  assert.equal(await rollbackMigration('012_espn_connection_security'), '012_espn_connection_security');
  assert.equal(row(`SELECT COUNT(*) n FROM sqlite_master WHERE type='table' AND name='espn_credentials'`).n, 0);
  assert.equal(await rollbackMigration('011_espn_credential_health'), '011_espn_credential_health');
  ...
  assert.deepEqual(await runMigrations(), ['009_authoritative_actor_and_ownership_guards', '010_news_provenance_and_dedup', '011_espn_credential_health', '012_espn_connection_security', '013_espn_live_draft_setup', '014_espn_authoritative_board', '015_espn_durable_sync']);
  for (const trigger of ['model_experiments_promoter_required_insert', 'model_experiments_promoter_required_update',
    'validate_draft_team_owner_update', 'validate_draft_ownership_parent_update']) {
  ...
  assert.ok(row(`SELECT name FROM sqlite_master WHERE type='table' AND name='espn_credentials'`));
  ...
  assert.deepEqual(await runMigrations(), ['001_baseline_marker', '002_platform_audit_log', '003_draft_state_machine', '004_model_lab', '005_model_registry_integrity', '006_identity_and_draft_authorization', '007_model_permissions_and_upgrade_guard', '008_model_actor_foreign_keys', '009_authoritative_actor_and_ownership_guards', '010_news_provenance_and_dedup', '011_espn_credential_health', '012_espn_connection_security', '013_espn_live_draft_setup', '014_espn_authoritative_board', '015_espn_durable_sync']);
  assert.ok(row(`SELECT name FROM schema_migrations WHERE name='005_model_registry_integrity'`));
