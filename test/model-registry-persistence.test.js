...
test('latest migration down and re-up are transactional and reproducible', async () => {
  ...
  assert.deepEqual(await runMigrations(), ['009_authoritative_actor_and_ownership_guards', '010_news_provenance_and_dedup', '011_league_connection_integrity', '012_teaser_execution_ledger', '013_news_read_path_indexes', '014_profit_execution_triggers', '015_manager_profiles', '016_mock_draft_ownership_exemption']);
  ...
  assert.deepEqual(await runMigrations(), ['001_baseline_marker', '002_platform_audit_log', '003_draft_state_machine', '004_model_lab', '005_model_registry_integrity', '006_identity_and_draft_authorization', '007_model_permissions_and_upgrade_guard', '008_model_actor_foreign_keys', '009_authoritative_actor_and_ownership_guards', '010_news_provenance_and_dedup', '011_league_connection_integrity', '012_teaser_execution_ledger', '013_news_read_path_indexes', '014_profit_execution_triggers', '015_manager_profiles', '016_mock_draft_ownership_exemption']);
  assert.ok(row(`SELECT name FROM schema_migrations WHERE name='005_model_registry_integrity'`));
