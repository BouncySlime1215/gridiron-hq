...
test('the refresh loop runs the audit after manager signals, and the web server never imports the collector', async () => {
  ...
  // Integration order (INTEGRATION-AUDIT-0923 section 2): number_audit, then EVAL-01's brain_report,
  // then TELLS-01b's tells producer, which reads the manager signals built before it.
  assert.deepEqual(order.slice(-4), ['build-manager-signals.mjs', 'number_audit', 'run-graders.mjs', 'engine-tells.mjs']);
