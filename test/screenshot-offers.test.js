...
const M106 = await import('../server/migrations/106_trade_outcomes_screenshot_source.js');
...
test('a fresh database allows the observed_screenshot source (migration 106)', () => {
...
test('an existing database with child rows is widened by the preflight repair, rows, ids and children intact', () => {
  ...
  d.exec('BEGIN IMMEDIATE'); M106.up(d); d.exec('COMMIT');     // child rows: 106 leaves it to preflight
